...
# Example:
nmcli device wifi list | grep "BPI-RDKB-MLO-AP-xxxxxx"
```
> Replace `xxxxxx` with the last 6 digits of the device MAC address.
...
# Example:
nmcli device wifi connect BPI-RDKB-MLO-AP-xxxxxx password <PASSWORD>
```
> Replace `xxxxxx` with the last 6 digits of the device MAC address.
