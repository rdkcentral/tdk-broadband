...
ip = <ipaddress>
port = <port>
obj1.configureTestCase(ip,port,'TS_STABILITY_MultipleReboots')
obj2.configureTestCase(ip,port,'TS_STABILITY_MultipleReboots')
...
if "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper():
    ...
    if not testFailed:
        for iteration in range(1, TOTAL_ITERATIONS + 1):
            ...
            print("\n" + "=" * 90)
            print(f"\nRebooting the device: {iteration}")
            print("=" * 90)
            #Reboot the DUT
            doRebootDUT(obj1)
    ...
    if iterationFailed:
        ...
        success, artifact_details = collect_failure_artifacts(obj1, stability_type, iteration, failureReason, UPLOAD_SERVER_URL, FAILURE_ARTIFACT_ROOT)
