...
if "SUCCESS" in loadmodulestatus_sys.upper() and "SUCCESS" in loadmodulestatus.upper():
    ...
    tdkTestObj, actualresult, wan_ip = get_target_wan_interface(sysobj, ANDROID_WAN_INTERFACE)
    if expectedresult in actualresult:
        tdkTestObj.setResultStatus("SUCCESS")
        print("ACTUAL RESULT %d: WAN interface %s has IP address assigned: %s" % (step, ANDROID_WAN_INTERFACE, wan_ip))
        print("[TEST EXECUTION RESULT] : SUCCESS")

        step += 1
        # Step 2: Get the MAC address of the WAN interface
        print("\nTEST STEP %d: Get the MAC address of the WAN interface %s" % (step, ANDROID_WAN_INTERFACE))
        print("EXPECTED RESULT %d: Should successfully retrieve MAC address" % step)
        tdkTestObj, actualresult, wan_mac = get_interface_mac_address(sysobj, ANDROID_WAN_INTERFACE)
        if expectedresult in actualresult:
            tdkTestObj.setResultStatus("SUCCESS")
