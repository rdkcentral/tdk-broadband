...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_RNDIS_VerifyHostTableUpdateWithWiFiClient')
obj.configureTestCase(ip,port,'TS_RNDIS_VerifyHostTableUpdateWithWiFiClient')
    ...
    if expectedresult in actualresult:
        ...
        if expectedresult in actualresult:
            ...
            if hostEntries.isdigit():
                ...
                # Step 3 & 4: Iterate through the Host Table and find WiFi client entry
                clientDetected = 0
                clientIndex = -1
                detectedSSID = ""
                for index in range(1, hostEntries + 1):
                    ...
                    if expectedresult in actualresult:
                        ...
                        # Check if Layer1Interface is either Device.WiFi.SSID.1 or Device.WiFi.SSID.2
                        if layer1Interface == EXPECTED_LAYER1_INTERFACE_WIFI_SSID1 or layer1Interface == EXPECTED_LAYER1_INTERFACE_WIFI_SSID2:
                            clientDetected = 1
                            clientIndex = index
                            detectedSSID = layer1Interface
                            print("Identified the Host Table Entry for WiFi client as : %d" % index)
                            print("WiFi client connected to : %s" % layer1Interface)

                            # Step 5: Check if the WiFi client is shown as active
                            step += 1
                            ...
                            if expectedresult in actualresult:
                                ...
                                if activeStatus == "true":
                                    tdkTestObj_tr181.setResultStatus("SUCCESS")
                                    print("Host is Active - WiFi client detected in Host Table (%s) while RNDIS is active" % detectedSSID)
                                    break
                                else:
                ...
                if clientDetected != 1:
                    step += 1
                    tdkTestObj_tr181.setResultStatus("FAILURE")
                    print("\nTEST STEP %d: Verify WiFi client entry exists in Host Table" % step)
                    print("EXPECTED RESULT %d: At least one Host Table entry should have Layer1Interface as Device.WiFi.SSID.1 or Device.WiFi.SSID.2" % step)
                    print("ACTUAL RESULT %d: None of the Host Table entries show Device.WiFi.SSID.1 or Device.WiFi.SSID.2 as Layer 1 Interface" % step)
                    print("[TEST EXECUTION RESULT] : FAILURE")
            else:
                tdkTestObj_tr181.setResultStatus("FAILURE")
