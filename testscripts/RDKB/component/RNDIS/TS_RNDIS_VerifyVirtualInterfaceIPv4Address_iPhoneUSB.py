...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_RNDIS_VerifyVirtualInterfaceIPv4Address_iPhoneUSB')
obj.configureTestCase(ip,port,'TS_RNDIS_VerifyVirtualInterfaceIPv4Address_iPhoneUSB')
...
if "SUCCESS" in loadmodulestatus_sys.upper() and "SUCCESS" in loadmodulestatus.upper():
    ...
    tdkTestObj, actualresult, interface_ip = get_target_wan_interface(sysobj, IOS_WAN_INTERFACE)
    if expectedresult in actualresult:
        tdkTestObj.setResultStatus("SUCCESS")
        print("ACTUAL RESULT %d: WAN interface %s has active IP address: %s" % (step, IOS_WAN_INTERFACE, interface_ip))
        ...
        if expectedresult in actualresult and details != "":
            ...
            print("\nTEST STEP %d: Compare virtual interface IPv4 address with %s interface IP" % (step, IOS_WAN_INTERFACE))
            ...
            if dm_ipv4_address == interface_ip:
                ...
                print("ACTUAL RESULT %d: IP addresses match - DM IPv4: %s, %s IP: %s" % 
                      (step, dm_ipv4_address, IOS_WAN_INTERFACE, interface_ip))
                print("[TEST EXECUTION RESULT] : SUCCESS")
            else:
