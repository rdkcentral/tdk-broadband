...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_RNDIS_VerifyHostTableUpdateWithWiFiClient_AndroidUSB')
obj.configureTestCase(ip,port,'TS_RNDIS_VerifyHostTableUpdateWithWiFiClient_AndroidUSB')
...
if "SUCCESS" in loadmodulestatus_sys.upper() and "SUCCESS" in loadmodulestatus.upper():
    ...
    # Verify the target Android WAN interface has an active IP address
    if proceed_flag == 1:
        step += 1
        print("\nTEST STEP %d: Verify the target WAN interface %s has an active IP address" %(step,ANDROID_WAN_INTERFACE))
        print("EXPECTED RESULT %d: Interface %s should have an active IP address" %(step,ANDROID_WAN_INTERFACE))
        ...
        if expectedresult in actualresult and details != "":
            tdkTestObj.setResultStatus("SUCCESS")
            print("ACTUAL RESULT %d: WAN interface %s has an active IP address: %s" %(step,ANDROID_WAN_INTERFACE,details))
            print("[TEST EXECUTION RESULT] : SUCCESS")
        else:
            proceed_flag = 0
            tdkTestObj.setResultStatus("FAILURE")
            print("ACTUAL RESULT %d: Failed to verify that WAN interface %s has an active IP address. Details: %s" %(step,ANDROID_WAN_INTERFACE,details))
            print("[TEST EXECUTION RESULT] : FAILURE")
