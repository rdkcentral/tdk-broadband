...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_RNDIS_VerifyCellularDataInterface_iPhoneUSB')
obj.configureTestCase(ip,port,'TS_RNDIS_VerifyCellularDataInterface_iPhoneUSB')
...
if "SUCCESS" in loadmodulestatus_sys.upper() and "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        if expectedresult in actualresult and details != "":
            ...
            print("EXPECTED RESULT %d: Data interface should match %s" % (step, IOS_WAN_INTERFACE))
            if data_interface == IOS_WAN_INTERFACE:
                tdkTestObj_tr181.setResultStatus("SUCCESS")
                print("ACTUAL RESULT %d: Data interface matches - DM value: %s, Expected: %s" % 
                      (step, data_interface, IOS_WAN_INTERFACE))
                print("[TEST EXECUTION RESULT] : SUCCESS")
            else:
