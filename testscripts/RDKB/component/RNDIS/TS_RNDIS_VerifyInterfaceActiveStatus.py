...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_RNDIS_VerifyInterfaceActiveStatus')
obj.configureTestCase(ip,port,'TS_RNDIS_VerifyInterfaceActiveStatus')
...
if "SUCCESS" in loadmodulestatus_sys.upper() and "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        step += 1
        # Step 2: Get Device.X_RDK_WanManager.InterfaceActiveStatus
        ...
        if expectedresult in actualresult:
            interface_active_status = details.split("VALUE:")[1].split(' ')[0].strip() if "VALUE:" in details else details.strip()
            tdkTestObj_tr181.setResultStatus("SUCCESS")
            print("ACTUAL RESULT %d: Interface active status is: %s" % (step, interface_active_status))
            print("[TEST EXECUTION RESULT] : SUCCESS")

            step += 1
            # Step 3: Confirm the value is LTE,1|WanOE,0
