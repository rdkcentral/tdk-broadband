...
def loadAndUnloadModules():
    ...
    ip = <ipaddress>
    port = <port>
    obj.configureTestCase(ip,port,'TS_FirmwareUpgrade_InvalidFWUpgradeUsingXCONFServer')
...
if load_flag == 1:
    ...
    if "FAILURE" not in actualresult_all:
        ...
        step += 1
        #Target firmware details are set to dummy values for invalid firmware upgrade test. These values do not exist on the server.
        ...
        if FirmwareFilename != Old_FirmwareFilename and FirmwareFilename and erouter_ip != "":
            ...
            if "FAILURE" not in result and "" not in cmd_details:
                ...
                if expectedresult in actualresult:
                    ...
                    print(f"TEST STEP {step}: Check whether the firmware download is triggered in the device when invalid firmware is provided")
                    ...
                    if expectedresult in actualresult and details.strip() == "":
                        tdkTestObj.setResultStatus("SUCCESS")
                        print(f"ACTUAL RESULT {step}: Firmware download is not triggered as expected since invalid firmware is provided. Return Status: {details.strip()}")
                        print("[TEST EXECUTION RESULT] : SUCCESS\n")
                    else:
                        tdkTestObj.setResultStatus("FAILURE")
                        print(f"ACTUAL RESULT {step}: Firmware download is triggered unexpectedly. Return Status: {details.strip()}")
