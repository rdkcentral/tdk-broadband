...
if load_flag == 1:
    ...
    step = 1
    #Disable the firmware upgrade cron job to avoid automatic firmware upgrade during the test execution
    cron_disable_flag = manageFirmwareUpgradeCronJob(obj, step, enable=False)
    if cron_disable_flag:
        step += 1
        ...
        # Re-enable the firmware upgrade cron job after the test execution
        step += 1
        cron_enable_flag = manageFirmwareUpgradeCronJob(obj, step, enable=True)
    else:
        print("Failed to disable the firmware upgrade cron job. So skipping the test\n")
    #Unload the module
