...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <test_cases>
    <test_case_id>TC_TAD_96</test_case_id>
    <test_objective>Check if an on-demand Internet connectivity check is triggered by enabling the parameter X_RDK_DNSInternet.WANInterface.{i}.QueryNow when Device.Diagnostics.X_RDK_DNSInternet.WANInterface.{i}.Enable is enabled and Device.Diagnostics.X_RDK_DNSInternet.TestURL.{i}.URL is set to an empty URL. Also check if the DNS query result retrieved using Device.Diagnostics.X_RDK_DNSInternet.WANInterface.{i}.QueryNowResult is in "DISCONNECTED" state (state = 2).</test_objective>
...
10. Now set a new Test URL " "(empty) after creating a table instance Device.Diagnostics.X_RDK_DNSInternet.TestURL. and validate with get.
...
16. Revert the WAN interface enable state if required using Device.Diagnostics.X_RDK_DNSInternet.WANInterface.{i}.Enable.
17. Revert to the initial Test URL configuration if required.
18. Revert to the initial Device.Diagnostics.X_RDK_DNSInternet.Enable state if required.</automation_approch>
    <expected_output>On-demand Internet connectivity check should be triggered by enabling the parameter Device.Diagnostics.X_RDK_DNSInternet.WANInterface.{i}.QueryNow when Device.Diagnostics.X_RDK_DNSInternet.WANInterface.{i}.Enable is enabled and Device.Diagnostics.X_RDK_DNSInternet.TestURL.{i}.URL is set to an empty URL. Also, the DNS query result retrieved using Device.Diagnostics.X_RDK_DNSInternet.WANInterface.{i}.QueryNowResult should be in "DISCONNECTED" state (state = 2).</expected_output>
    <priority>High</priority>
    <test_stub_interface>tad</test_stub_interface>
    <test_script>TS_TAD_CheckDNSInternetConnectivity_QueryResultWithEmptyTestURL</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_TAD_CheckDNSInternetConnectivity_QueryResultWithEmptyTestURL');
tr181obj.configureTestCase(ip,port,'TS_TAD_CheckDNSInternetConnectivity_QueryResultWithEmptyTestURL');
...
if "SUCCESS" in loadmodulestatus.upper()and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if preReqStatus == 0:
        ...
        if testURLPreReq == 0:
            print("Configure an empty URL to the Test URL table");
            #Set the Test URL to " "(empty) to a newly created Test URL table instance
            step = step + 1;
            testURLList = [" "];
                    ...
                    for wanInterface in range(1, numberOfInterfaces + 1):
                        ...
                        if proceedFlag == 1:
                            ...
                            if expectedresult in actualresult:
                                ...
                                step = step + 1;
                                print("\nTEST STEP %d : Check if the DNS query result status is DISCONNECTED when an empty Test URL is configured" %step);
                                print("EXPECTED RESULT %d : The DNS query result status should be DISCONNECTED when an empty Test URL is configured" %step);
