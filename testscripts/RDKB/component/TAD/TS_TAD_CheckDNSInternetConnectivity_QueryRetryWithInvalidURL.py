...
14. Set Device.Diagnostics.X_RDK_DNSInternet.WANInterface.{i}.QueryRetry to a new value which is +1 of the initial value. Validate with get.
...
16. Sleep for retry + 5 seconds before checking the DNS Query result status.
...
18. Check if the log "Sending Query Retry Attempt:[New Retry Count Set]" is populated under /rdklogs/logs/DNSInternetCheck.txt.0. Else return failure.
                            ...
                            if expectedresult in actualresult and details != "":
                                ...
                                if details.isdigit():
                                    ...
                                    if expectedresult in actualresult and details != "":
                                        ...
                                        #Find the initial number of log lines indicating the DNS query retry attempts
                                        step = step + 1;
                                        tdkTestObj = sysobj.createTestStep('ExecuteCmd');
                                        retryLog = "Sending Query Retry Attempt:" + str(setRetry);
                                        ...
                                        if expectedresult in actualresult:
                                            ...
                                            #Check the DNS Quert result status after waiting for the retries to be completed
                                            sleepTime = setRetry + 5;
                                            ...
                                            if expectedresult in actualresult and details == "DISCONNECTED":
                                                ...
                                                #Check if the log line difference is >= 1 (accounting for all the DNS servers configured)
                                                print "The Retry Logs should be populated under %s" %file;
                                                step = step + 1;
                                                print "\nTEST STEP %d : Check if the log - %s is populated under %s as the URL configured in invalid" %(step, retryLog, file);
                                                print "EXPECTED RESULT %d : Log should be present under %s as the URL configured is invalid" %(step, file);
