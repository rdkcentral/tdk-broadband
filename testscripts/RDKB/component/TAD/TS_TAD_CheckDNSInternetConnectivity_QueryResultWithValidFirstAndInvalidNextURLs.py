...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <test_cases>
    <test_case_id>TC_TAD_92</test_case_id>
    <test_objective>Check if the on-demand Internet connectivity check concludes with success and X_RDK_DNSInternet.WANInterface.{i}.QueryNowResult set to '1' (CONNECTED) for the first test URL entry within the X_RDK_DNSInternet.TestURL.{i}. table which is resolved successfully. Also check if the required logging is seen under DNSInternetCheck.txt.0 for the valid URL.</test_objective>
...
ParamValue : "www.google.com", "www.invalidurl.com"
Type : string
...
10. Now set new Test URLs "www.google.com" and "www.invalidurl.com" after creating new table instances using Device.Diagnostics.X_RDK_DNSInternet.TestURL. and validate with get.
...
16. Check if the log line "QueryNow Succeeded for URL www.google.com" is populated under /rdklogs/logs/DNSInternetCheck.txt.0 and "Resolution Failed for URL www.invalidurl.com" should not be populated. This is because the DNS query is success with the first valid test URL configured and hence does not start DNS queries with next available URLs.
...
19. Revert to the initial Device.Diagnostics.X_RDK_DNSInternet.Enable state if required.</automation_approch>
    <expected_output>The on-demand Internet connectivity check should conclude with success and X_RDK_DNSInternet.WANInterface.{i}.QueryNowResult set to '1' (CONNECTED) for the first test URL entry within the X_RDK_DNSInternet.TestURL.{i}. table which is resolved successfully. Also the required logging should be seen under DNSInternetCheck.txt.0 for the valid URL which is configured.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tad</test_stub_interface>
    <test_script>TS_TAD_CheckDNSInternetConnectivity_QueryResultWithValidFirstAndInvalidNextURLs</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_TAD_CheckDNSInternetConnectivity_QueryResultWithValidFirstAndInvalidNextURLs');
tr181obj.configureTestCase(ip,port,'TS_TAD_CheckDNSInternetConnectivity_QueryResultWithValidFirstAndInvalidNextURLs');
sysobj.configureTestCase(ip,port,'TS_TAD_CheckDNSInternetConnectivity_QueryResultWithValidFirstAndInvalidNextURLs');
...
if "SUCCESS" in loadmodulestatus.upper()and "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper():
    ...
    if preReqStatus == 0:
        ...
        if testURLPreReq == 0:
            #Set the Test URL to "www.google.com" and "www.invalidurl.com" to newly created Test URL table instances
            step = step + 1;
            testURLList = ["www.google.com", "www.invalidurl.com"];
            ...
            if setTestURL == 0:
                ...
                if numberOfInterfaces >= 1:
                    for wanInterface in range(1, numberOfInterfaces + 1):
                        ...
                        if proceedFlag == 1:
                            #Find the initial number of log lines indicating successful DNS query
                            step = step + 1;
                            tdkTestObj = sysobj.createTestStep('ExecuteCmd');
                            successQuery = "QueryNow Succeeded for URL " + testURLList[0];
                            ...
                            #Find the initial number of log lines indicating failed DNS query
                            step = step + 1;
                            failedQuery = "Resolution Failed for URL " + testURLList[1];
                            print("\nGet the initial number of log lines of " + failedQuery);
                            failure_count_initial = getLogFileTotalLinesCount(tdkTestObj, file, failedQuery, step);
                            ...
                            if expectedresult in actualresult:
                                ...
                                #Check the final DNS Query result status as one of the Test URLs configured is valid
                                ...
                                if expectedresult in actualresult and details == "CONNECTED":
                                    ...
                                    failure_count_final = getLogFileTotalLinesCount(tdkTestObj, file, failedQuery, step);\
                                    ...
                                    step = step + 1;
                                    print("TEST STEP %d : Check if the log - %s is populated under %s after successful DNS query" %(step, successQuery, file));
                                    ...
                                    print("\nNOT expecting to see the failure query log line count incremented by 1 as the DNS query result \"CONNECTED\" is already populated with the first valid Test URL");
                                    step = step + 1;
                                    print("TEST STEP %d : Check if the log - %s is populated under %s after the first successful DNS query" %(step, failedQuery, file));
                                    print("EXPECTED RESULT %d : Log should NOT be present under %s after the first successful DNS query" %(step, file));

                                    print("Number of initial log lines of %s : %d" %(failedQuery, failure_count_initial));
                                    print("Number of final log lines of %s : %d" %(failedQuery, failure_count_final));

                                    if failure_count_final == failure_count_initial:
                                        #Set the result status of execution
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("ACTUAL RESULT %d : The required log lines are NOT found under %s after the first successful DNS query" %(step, file));
                                        #Get the result of execution
                                        print("[TEST EXECUTION RESULT] : SUCCESS");
                                    else:
                                        #Set the result status of execution
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("ACTUAL RESULT %d : The required log lines are found under %s after the first successful DNS query" %(step, file));
                                        #Get the result of execution
                                        print("[TEST EXECUTION RESULT] : FAILURE");
                                else:
                                    #Set the result status of execution
