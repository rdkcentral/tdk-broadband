...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <test_cases>
    <test_case_id>TC_TAD_93</test_case_id>
    <test_objective>Check if the on-demand Internet connectivity check concludes with success and X_RDK_DNSInternet.WANInterface.{i}.QueryNowResult set to '1' (CONNECTED) for the first test URL entry within the X_RDK_DNSInternet.TestURL.{i}. table which is resolved successfully. Also check if the required logging is seen under DNSInternetCheck.txt.0 for the valid URL and not seen for the second valid Test URL.</test_objective>
...
ParamValue : "www.google.com", "www.amazon.com"
Type : string
...
10. Now set new Test URLs "www.google.com" and "www.amazon.com" after creating new table instances using Device.Diagnostics.X_RDK_DNSInternet.TestURL. and validate with get.
...
14. Sleep for 5 seconds before checking the DNS Query result status.
...
16. Check if the log line "QueryNow Succeeded for URL www.google.com" is populated under /rdklogs/logs/DNSInternetCheck.txt.0 and "QueryNow Succeeded for URL www.amazon.com" should not be populated. This is because the DNS query is success with the first valid test URL configured and hence does not start DNS queries with next available URLs even if they are valid.
...
19. Revert to the initial Device.Diagnostics.X_RDK_DNSInternet.Enable state if required.</automation_approch>
    <expected_output>The on-demand Internet connectivity check should conclude with success and X_RDK_DNSInternet.WANInterface.{i}.QueryNowResult set to '1' (CONNECTED) for the first test URL entry within the X_RDK_DNSInternet.TestURL.{i}. table which is resolved successfully. Also, the required logging should be seen under DNSInternetCheck.txt.0 for the valid URL which is configured and not seen for the second valid Test URL.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tad</test_stub_interface>
    <test_script>TS_TAD_CheckDNSInternetConnectivity_QueryResultWithTwoValidURLs</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_TAD_CheckDNSInternetConnectivity_QueryResultWithTwoValidURLs');
tr181obj.configureTestCase(ip,port,'TS_TAD_CheckDNSInternetConnectivity_QueryResultWithTwoValidURLs');
sysobj.configureTestCase(ip,port,'TS_TAD_CheckDNSInternetConnectivity_QueryResultWithTwoValidURLs');
...
if "SUCCESS" in loadmodulestatus.upper()and "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper():
    ...
    if preReqStatus == 0:
        ...
        if testURLPreReq == 0:
            #Set the Test URL to "www.google.com" and "www.amazon.com" to newly created Test URL table instances
            step = step + 1;
            testURLList = ["www.google.com", "www.amazon.com"];
            ...
            if setTestURL == 0:
                ...
                if numberOfInterfaces >= 1:
                    for wanInterface in range(1, numberOfInterfaces + 1):
                        ...
                        if proceedFlag == 1:
                            #Find the initial number of log lines indicating the first successful DNS query
                            step = step + 1;
                            tdkTestObj = sysobj.createTestStep('ExecuteCmd');
                            successQuery1 = "QueryNow Succeeded for URL " + testURLList[0];
                            print("\nGet the initial number of log lines of " + successQuery1);
                            file = "/rdklogs/logs/DNSInternetCheck.txt.0";
                            success_count_initial_url1 = getLogFileTotalLinesCount(tdkTestObj, file, successQuery1, step);

                            #Find the initial number of log lines indicating the second successful DNS query
                            step = step + 1;
                            successQuery2 = "QueryNow Succeeded for URL " + testURLList[1];
                            print("\nGet the initial number of log lines of " + successQuery2);
                            success_count_initial_url2 = getLogFileTotalLinesCount(tdkTestObj, file, successQuery2, step);
                            ...
                            if expectedresult in actualresult:
                                ...
                                #Check the final DNS Query result status as both of the Test URLs configured is valid
                                ...
                                step = step + 1;
                                print("\nTEST STEP %d : Check if the DNS query result status is CONNECTED as the first Test URL configured is valid" %step);
                                print("EXPECTED RESULT %d : The DNS query result status should be CONNECTED as the first Test URL configured is valid" %step);
                                ...
                                if expectedresult in actualresult and details == "CONNECTED":
                                    ...
                                    #Find the final number of log lines indicating first successful DNS query
                                    step = step + 1;
                                    tdkTestObj = sysobj.createTestStep('ExecuteCmd');
                                    print("\nGet the final number of log lines of " + successQuery1);
                                    success_count_final_url1 = getLogFileTotalLinesCount(tdkTestObj, file, successQuery1, step);

                                    #Find the final number of log lines indicating second successful DNS query
                                    step = step + 1;
                                    print("\nGet the final number of log lines of " + successQuery2);
                                    success_count_final_url2 = getLogFileTotalLinesCount(tdkTestObj, file, successQuery2, step);\

                                    #Check if the log line difference is 1
                                    print("\nAs the first Test URL is a valid one, we expect to see the successQuery log line count incremented by 1")
                                    step = step + 1;
                                    print("TEST STEP %d : Check if the log - %s is populated under %s after successful DNS query" %(step, successQuery1, file));
                                    print("EXPECTED RESULT %d : Log should be present under %s after successful DNS query" %(step, file));

                                    print("Number of initial log lines of %s : %d" %(successQuery1, success_count_initial_url1));
                                    print("Number of final log lines of %s : %d" %(successQuery1, success_count_final_url1));

                                    if success_count_final_url1 == (success_count_initial_url1 + 1):
                                        #Set the result status of execution
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("ACTUAL RESULT %d : The required log lines are found under %s after successful DNS query" %(step, file));
                                        #Get the result of execution
                                        print("[TEST EXECUTION RESULT] : SUCCESS");
                                    else:
                                        #Set the result status of execution
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("ACTUAL RESULT %d : The required log lines are NOT found under %s after successful DNS query" %(step, file));
                                        #Get the result of execution
                                        print("[TEST EXECUTION RESULT] : FAILURE");

                                    #Check if the log line difference is 0
                                    print("\nNOT expecting to see the log line count incremented by 1 for the second valid Test URL as the DNS query result \"CONNECTED\" is already populated with first valid Test URL");
                                    step = step + 1;
                                    print("TEST STEP %d : Check if the log - %s is populated under %s after first successful DNS query" %(step, successQuery2, file));
                                    print("EXPECTED RESULT %d : Log should NOT be present under %s after first successful DNS query" %(step, file));

                                    print("Number of initial log lines of %s : %d" %(successQuery2, success_count_initial_url2));
                                    print("Number of final log lines of %s : %d" %(successQuery2, success_count_final_url2));

                                    if success_count_final_url2 == success_count_initial_url2:
                                        #Set the result status of execution
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print("ACTUAL RESULT %d : The required log lines are NOT found under %s after first successful DNS query" %(step, file));
                                        #Get the result of execution
                                        print("[TEST EXECUTION RESULT] : SUCCESS");
                                    else:
                                        #Set the result status of execution
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print("ACTUAL RESULT %d : The required log lines are found under %s after first successful DNS query" %(step, file));
                                        #Get the result of execution
                                        print("[TEST EXECUTION RESULT] : FAILURE");
                                else:
                                    #Set the result status of execution
