...
<xml>
  ...
  <name>TS_TAD_CheckDNSInternetConnectivity_QueryResultWithTwoInvalidURLs</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if the on-demand Internet connectivity check concludes with failure and X_RDK_DNSInternet.WANInterface.{i}.QueryNowResult set to '2' (DISCONNECTED) for the two invalid test URL entries within the X_RDK_DNSInternet.TestURL.{i}. table which are not resolved successfully. Also check if the required logging is seen under DNSInternetCheck.txt.0 for both invalid URLs.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TAD_94</test_case_id>
    <test_objective>Check if the on-demand Internet connectivity check concludes with failure and X_RDK_DNSInternet.WANInterface.{i}.QueryNowResult set to '2' (DISCONNECTED) for the two invalid test URL entries within the X_RDK_DNSInternet.TestURL.{i}. table which are not resolved successfully. Also check if the required logging is seen under DNSInternetCheck.txt.0 for both invalid URLs.</test_objective>
...
ParamValue :"www.invalidurl1.com", "www.invalidurl2.com"
Type : string
...
10. Now set new Test URLs "www.invalidurl1.com" and  "www.invalidurl2.com" after creating new table instances using Device.Diagnostics.X_RDK_DNSInternet.TestURL. and validate with get.
...
15. Get Device.Diagnostics.X_RDK_DNSInternet.WANInterface.{i}.QueryNowResult and check if the value returned is 2 (for DISCONNECTED). Else return failure.
16. Check if the log line "Resolution Failed for URL www.invalidurl1.com" and "Resolution Failed for URL www.invalidurl2.com" is populated under /rdklogs/logs/DNSInternetCheck.txt.0. This is because the DNS query is failed with the first valid test URL configured and then goes to start the DNS queries with the second URL which also fails as both are invalid.
...
19. Revert to the initial Device.Diagnostics.X_RDK_DNSInternet.Enable state if required.</automation_approch>
    <expected_output>The on-demand Internet connectivity check should conclude with success and X_RDK_DNSInternet.WANInterface.{i}.QueryNowResult set to '2' (DISCONNECTED) for both test URL entries within the X_RDK_DNSInternet.TestURL.{i}. table as both are invalid and not resolved successfully. Also, the required logging should be seen under DNSInternetCheck.txt.0 for the both the invalid URLs configured.</expected_output>
    <priority>High</priority>
    <test_stub_interface>tad</test_stub_interface>
    <test_script>TS_TAD_CheckDNSInternetConnectivity_QueryResultWithTwoInvalidURLs</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_TAD_CheckDNSInternetConnectivity_QueryResultWithTwoInvalidURLs');
tr181obj.configureTestCase(ip,port,'TS_TAD_CheckDNSInternetConnectivity_QueryResultWithTwoInvalidURLs');
sysobj.configureTestCase(ip,port,'TS_TAD_CheckDNSInternetConnectivity_QueryResultWithTwoInvalidURLs');
...
if "SUCCESS" in loadmodulestatus.upper()and "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper():
    ...
    if preReqStatus == 0:
        ...
        if testURLPreReq == 0:
            #Set the Test URL to "www.invalidurl1.com" and "www.invalidurl2.com" to newly created Test URL table instances
            step = step + 1;
            testURLList = ["www.invalidurl1.com", "www.invalidurl2.com"];
            ...
            if setTestURL == 0:
                ...
                if numberOfInterfaces >= 1:
                    for wanInterface in range(1, numberOfInterfaces + 1):
                        ...
                        if proceedFlag == 1:
                            #Find the initial number of log lines indicating the first failed DNS query
                            step = step + 1;
