...
tr181obj.configureTestCase(ip,port,'TS_TelcoVoiceManager_GetVoiceProfileLineStatusAfterOutboundCallConfiguration')
...
if expectedresult in loadmodulestatus.upper():
    ...
    if get_flag:
        ...
        if set_flag:
            ...
            print(f"EXPECTED RESULT {step}: Should get the Line Status once the configurations are updated")
            tdkTestObj, actualresult, status = getLineStatus(tr181obj)
            if expectedresult in actualresult and "Up" in status:
                tdkTestObj.setResultStatus("SUCCESS")
                print(f"ACTUAL RESULT {step}: Successfully got the Line Status once the configurations are updated as {status}")
                print("[TEST EXECUTION RESULT] : SUCCESS")
            else:
                tdkTestObj.setResultStatus("FAILURE")
                print(f"ACTUAL RESULT {step}: Failed to get the Line Status once the configurations are updated. Status: {status}")
