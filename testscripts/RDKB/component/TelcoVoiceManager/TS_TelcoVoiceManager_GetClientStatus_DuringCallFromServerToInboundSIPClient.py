...
obj.configureTestCase(ip,port,'TS_TelcoVoiceManager_GetClientStatus_DuringCallFromServerToInboundSIPClient')
...
if expectedresult in loadmodulestatus.upper():
    ...
    print(f"Prerequisite : One SIP client need to be activated within the same WAN network using the default usernames and passwords specified in {pjsip_conf_file}.")
    ...
    if hangup_status:
        ...
        if initiate_call_status:
            ...
            print(f"\nTEST STEP {step}: Check whether the call from Asterisk Server to SIP client - {client1_username} is in progress.")
            ...
            if expectedresult in actualresult and "In use" in status:
                ...
                print(f"ACTUAL RESULT {step}: The call is in progress and SIP client status is {status}")
                ...
                print(f"ACTUAL RESULT {step}: Failed to connect the call and the SIP client status is {status}")
                print("[TEST EXECUTION RESULT] : FAILURE")
        else:
            print("Failed to initiate the call from the asterisk server to the inbound SIP client.")
    else:
