...
obj.configureTestCase(ip,port,'TS_TelcoVoiceManager_InvalidSIPClient_ValidateActiveInboundCallCount')
...
if expectedresult in loadmodulestatus.upper():
    ...
    if hangup_status:
        ...
        if expectedresult in actualresult and type(initial_call_count) is int and initial_call_count == 0:
            ...
            step +=1
            # Call Initiation from valid to invalid inbound SIP Clients
            print("\nInitiating a call between valid to invalid inbound SIP Clients.")
            ...
            if initiate_call_status:
                ...
                print(f"EXPECTED RESULT {step}: Active call count should remain zero after call initiation, indicating failed call.")
                tdkTestObj, actualresult, call_count = getActiveCallCount(obj)
                if expectedresult in actualresult  and type(call_count) is int and call_count == 0:
                    tdkTestObj.setResultStatus("SUCCESS")
                    print(f"ACTUAL RESULT {step}: The active call count remained zero after call initiation, indicating failed call. Current active call count is {call_count}")
                    print("[TEST EXECUTION RESULT] : SUCCESS")
                else:
                    tdkTestObj.setResultStatus("FAILURE")
                    print(f"ACTUAL RESULT {step}: The active call count is not 0 after call initiation. Current active call count is {call_count}")
                    print("[TEST EXECUTION RESULT] : FAILURE")
            else:
                print("Failed to initiate the call between valid to invalid inbound SIP Clients")
        else:
            tdkTestObj.setResultStatus("FAILURE")
