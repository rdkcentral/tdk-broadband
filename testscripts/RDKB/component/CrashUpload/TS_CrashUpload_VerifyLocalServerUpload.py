...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_CrashUpload_VerifyLocalServerUpload')
sysobj.configureTestCase(ip,port,'TS_CrashUpload_VerifyLocalServerUpload')
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus_sys.upper():
    ...
    old_pid = ""
    dmp_filename = ""
    initial_crash_portal_url = ""  # Variable to store initial value
    url_changed = False  # Flag to track if URL was changed
    ...
    if expectedresult in actualresult and initial_crash_portal_url:
        ...
        if expectedresult in actualresult:
            url_changed = True  # Mark that URL was successfully changed
            tdkTestObj.setResultStatus("SUCCESS")
            ...
            if actualresult in expectedresult and LOCAL_UPLOAD_URL in current_value.strip():
                ...
                if expectedresult in actualresult and details == "unlimited":
                    ...
                    if expectedresult in actualresult and "waiting" in state.lower():
                        ...
                        step += 1
                        # Check if /minidumps directory is empty
                            ...
                            if expectedresult in actualresult:
                                ...
                                try:
                                    ...
                                    if expectedresult in actualresult and dmp_filename != "":
                                        ...
                                        command = f"grep 'DEBUG S3 File Name' {CORE_LOG_TXT} | tail -n 1"
                                        tdkTestObj = sysobj.createTestStep('ExecuteCmd')
                                        tdkTestObj.addParameter("command", command)
                                        tdkTestObj.executeTestCase(expectedresult)
                                        actualresult = tdkTestObj.getResult()
                                        log_details = tdkTestObj.getResultDetails().strip()
                                        # Extract the .tgz filename from log_details
                                        tgz_filename = ""
                                        if log_details:
                                            try:
                                                tgz_filename = log_details.split("DEBUG S3 File Name : ")[1].strip().split(".dmp.tgz")[0]
                                            except IndexError:
                                                tgz_filename = log_details.strip()
                                        if expectedresult in actualresult and base_dmp_filename in tgz_filename:
                                            tdkTestObj.setResultStatus("SUCCESS")
