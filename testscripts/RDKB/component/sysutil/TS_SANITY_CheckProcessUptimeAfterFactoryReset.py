...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_SANITY_CheckProcessUptimeAfterFactoryReset')
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult:
                ...
                if "up" in parts:
                    ...
                    raw_val = parts[index + 1]
                    try:
                        uptime_min = int(raw_val)
                        upTime = uptime_min * 60
                        print(f"Uptime in seconds is {upTime}")
                    except Exception as e:
                        print(f"Failed to convert uptime output value '{raw_val}' to int. Exception occured: {e}")
                        upTime = None
                else:
                    print("Error: 'up' keyword not found in uptime output")
                    upTime = None
                ...
                if upTime is not None:
                    ...
                    print(f"Unable to parse the uptime. Not proceeding further")
                    tdkTestObj.setResultStatus("FAILURE")
            else:
                tdkTestObj.setResultStatus("FAILURE")
                print(f"ACTUAL RESULT {step}:Failed to get uptime of DUT")
                print("[TEST EXECUTION RESULT] : FAILURE")
        else:
            tdkTestObj.setResultStatus("FAILURE")
