...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_WEBCONFIG_PAMValidationUsingLanSubdoc</name>
  <primitive_test_id/>
  <primitive_test_name>Webconfig_DoNothing</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Validate PAM via Webconfig Feature using LAN subdoc</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WEBCONFIG_9</test_case_id>
    <test_objective>Validate PAM via Webconfig Feature using LAN subdoc</test_objective>
...
Device.DHCPv4.Server.Pool.{i}.MaxAddress.</input_parameters>
    ...
    4. Enter valid LAN subdoc information with new values in RDKM WebConfig Server.
    5. Set trigger parameter Device.X_RDK_WebConfig.ForceSync to root.
    6. Get the  Device.DHCPv4.Server.Pool.1.Enable, Device.DHCPv4.Server.Pool.1.IPRouters, Device.DHCPv4.Server.Pool.1.SubnetMask, Device.DHCPv4.Server.Pool.1.MinAddress,  Device.DHCPv4.Server.Pool.1.LeaseTime and Device.DHCPv4.Server.Pool.1.MaxAddress and validate whether it is updated.
    7. Unload the modules.</automation_approch>
    <expected_output>PAM validation via Webconfig Feature using LAN subdoc should be successful.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WEBCONFIG</test_stub_interface>
    <test_script>TS_WEBCONFIG_PAMValidationUsingLanSubdoc</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WEBCONFIG_PAMValidationUsingLanSubdoc')
sysobj.configureTestCase(ip,port,'TS_WEBCONFIG_PAMValidationUsingLanSubdoc')
            ...
            if "FAILURE" not in result:
                ...
                if expectedresult in actualresult and "Request successful" in details:
                    ...
                    if trigger_flag != 1:
                        ...
                        print("EXPECTED RESULT %d: The values of DHCPv4 parameters should be updated successfully" %step)

                        tdkTestObj = obj.createTestStep('TDKB_TR181Stub_Get')
                        tdkTestObj, result, final_dhcp_values = getMultipleParams(obj, dhcp_params)

                        final_dhcp_values[dhcp_params[-1]] = int(final_dhcp_values[dhcp_params[-1]])

                        if "FAILURE" not in result and list(final_dhcp_values.values()) == set_values:
                            tdkTestObj.setResultStatus("SUCCESS")
                            print("ACTUAL RESULT %d: The values of DHCPv4 parameters have been updated as per LAN subdoc successfully" %step)
                            print("[TEST EXECUTION RESULT] : SUCCESS")

                            step += 1
                            # Revert the DHCPv4 parameters to their initial values
                            print("\nTEST STEP %d: Execute curl command to revert the DHCPv4 parameters to their initial values" %step)
                            print("EXPECTED RESULT %d: The DHCPv4 parameters should be reverted successfully" %step)
                            ...
                            if expectedresult in actualresult and "Request successful" in details:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print("ACTUAL RESULT %d : Curl command executed successfully to revert the LAN subdoc. Details: %s" %(step,details))
                                ...
                                if trigger_flag != 1:
                                    ...
                                    step += 1
                                    print("\nTEST STEP %d: Validate the revert operation by getting the DHCPv4 parameters" %step)
                                    print("EXPECTED RESULT %d: The DHCPv4 parameters should be reverted to their initial values successfully" %step)
                                    tdkTestObj = obj.createTestStep('TDKB_TR181Stub_Get')
                                    ...
                                    if "FAILURE" not in result and dhcp_values == initial_dhcp_values:
                                        tdkTestObj.setResultStatus("SUCCESS")
                                        print("ACTUAL RESULT %d: The DHCPv4 parameters are reverted to their initial values successfully" %step)
                                        print("[TEST EXECUTION RESULT] : SUCCESS")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE")
                                        print("ACTUAL RESULT %d: Failed to revert the DHCPv4 parameters to their initial values" %step)
                                        print("[TEST EXECUTION RESULT] : FAILURE")
                                else:
                                    print("Failed to set the trigger value to %s" %value)
                            else:
                                tdkTestObj.setResultStatus("FAILURE")
                                print("ACTUAL RESULT %d : Curl command execution failed to revert the LAN subdoc. Details: %s" %(step, details))
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
                            print("ACTUAL RESULT %d: Failed to update the DHCPv4 parameters." %step)
                            print("[TEST EXECUTION RESULT] : FAILURE")
