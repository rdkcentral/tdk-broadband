...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_WEBCONFIG_6G_WIFIValidationUsingInvalidPrivateSSIDSubdoc</name>
  <primitive_test_id/>
  <primitive_test_name>Webconfig_DoNothing</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Validate WIFI via Webconfig Feature using invalid 6g privatessid subdoc</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WEBCONFIG_20</test_case_id>
    <test_objective>Validate WIFI via Webconfig Feature using invalid 6g privatessid subdoc</test_objective>
...
Device.WiFi.AccessPoint.{i}.Security.X_COMCAST-COM_KeyPassphrase</input_parameters>
    ...
    4. Enter 2.4g privatessid subdoc information with invalid values in RDKM WebConfig Server via curl command.
    ...
    <expected_output>WIFI validation via Webconfig Feature using invalid 6g privatessid subdoc should be failure.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WEBCONFIG</test_stub_interface>
    <test_script>TS_WEBCONFIG_6G_WIFIValidationUsingInvalidPrivateSSIDSubdoc</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WEBCONFIG_6G_WIFIValidationUsingInvalidPrivateSSIDSubdoc')
sysobj.configureTestCase(ip,port,'TS_WEBCONFIG_6G_WIFIValidationUsingInvalidPrivateSSIDSubdoc')
        ...
        if flag == 0:
            ...
            if expectedresult in actualresult and radio_entries in [2, 3]:
                ...
                if retrieval_flag != 1:
                    ...
                    #Getting the subdoc info with invalid values
                    ...
                    if expectedresult in actualresult and "Request successful" in details:
                        ...
                        if trigger_flag != 1:
                            ...
                            print("\nTEST STEP %d: Check whether the values of %s WiFi parameters fails to get updated as per invalid privatessid subdoc" %(step, wifi_radio))
                            ...
                            if "FAILURE" not in result and final_values != set_values:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print("ACTUAL RESULT %d: Failed to update the WiFi parameters as per invalid privatessid subdoc" %step)
                                print("[TEST EXECUTION RESULT] : SUCCESS")

                            else:
                                tdkTestObj.setResultStatus("FAILURE")
                                print("ACTUAL RESULT %d: The values of WiFi parameters have been updated as per invalid privatessid subdoc" %step)
                                print("[TEST EXECUTION RESULT] : FAILURE")
                                step += 1
