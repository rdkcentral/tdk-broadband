...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_WEBCONFIG_PAMValidationUsingInvalidLanSubdoc</name>
  <primitive_test_id/>
  <primitive_test_name>Webconfig_DoNothing</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Validate PAM via Webconfig Feature using invalid LAN subdoc</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WEBCONFIG_8</test_case_id>
    <test_objective>Validate PAM via Webconfig Feature using invalid LAN subdoc</test_objective>
...
Device.DHCPv4.Server.Pool.{i}.MaxAddress.</input_parameters>
    ...
    4. Enter invalid LAN subdoc information with new values in RDKM WebConfig Server.
    5. Set trigger parameter Device.X_RDK_WebConfig.ForceSync to root.
    6. Get the  Device.DHCPv4.Server.Pool.1.Enable, Device.DHCPv4.Server.Pool.1.IPRouters, Device.DHCPv4.Server.Pool.1.SubnetMask, Device.DHCPv4.Server.Pool.1.MinAddress,  Device.DHCPv4.Server.Pool.1.LeaseTime and Device.DHCPv4.Server.Pool.1.MaxAddress and validate whether it fails to update.
    7. Unload the modules.</automation_approch>
    <expected_output>PAM validation via Webconfig Feature using invalid LAN subdoc should fail.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WEBCONFIG</test_stub_interface>
    <test_script>TS_WEBCONFIG_PAMValidationUsingInvalidLanSubdoc</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WEBCONFIG_PAMValidationUsingInvalidLanSubdoc')
sysobj.configureTestCase(ip,port,'TS_WEBCONFIG_PAMValidationUsingInvalidLanSubdoc')
    ...
    if actualresult in expectedresult and mac != "":
        ...
        if flag == 0:
            ...
            if "FAILURE" not in result:
                ...
                print("\nTEST STEP %d: Execute the curl command to update %s subdoc to invalid values" %(step, subdoc_name))
                print("EXPECTED RESULT %d: The curl command execution should succeed" %step)
                ...
                if expectedresult in actualresult and "Request successful" in details:
                    ...
                    if trigger_flag != 1:
                        ...
                        if "FAILURE" not in result and final_dhcp_values == initial_dhcp_values:
                            tdkTestObj.setResultStatus("SUCCESS")
                            print("ACTUAL RESULT %d: The values of DHCPv4 parameters failed to get updated as per invalid LAN subdoc" %step)
                            print("[TEST EXECUTION RESULT] : SUCCESS")
                        else:
