...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_WEBCONFIG_PAMValidationUsingPartialPortForwardingSubdoc</name>
  <primitive_test_id/>
  <primitive_test_name>Webconfig_DoNothing</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Validate PAM via Webconfig Feature using partial PortForwarding subdoc</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WEBCONFIG_12</test_case_id>
    <test_objective>Validate PAM via Webconfig Feature using partial PortForwarding subdoc</test_objective>
...
Device.NAT.PortMapping.{i}.</input_parameters>
    <automation_approch>1. Load the modules
    2. Configure the webconfig settings if already not configured with webconfig server details.
    3  Get the Device.NAT. DM.
    4. Enter partial PortMapping subdoc information via WebConfig server using curl command.
    5. Set trigger parameter Device.X_RDK_WebConfig.ForceSync to root.
    6. Get the Device.NAT. and validate whether it fails to create the profile as per the subdoc configured.
    7. Unload the modules.</automation_approch>
    <expected_output>PAM validation via Webconfig Feature using PortForwarding subdoc fails to create the profiles cofigured.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WEBCONFIG</test_stub_interface>
    <test_script>TS_WEBCONFIG_PAMValidationUsingPartialPortForwardingSubdoc</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WEBCONFIG_PAMValidationUsingPartialPortForwardingSubdoc')
sysobj.configureTestCase(ip,port,'TS_WEBCONFIG_PAMValidationUsingPartialPortForwardingSubdoc')
    ...
    if expectedresult in actualresult:
        ...
        if actualresult in expectedresult and mac != "":
            ...
            if flag == 0:
                ...
                #Getting partial subdoc info
                subdoc_info = getPartialSubdocInfo(subdoc_info)
                ...
                print("EXPECTED RESULT %d: Failure in execution of curl command" %step)
                ...
                if expectedresult in actualresult and "Request successful" not in details:
                    tdkTestObj.setResultStatus("SUCCESS")
                    print("ACTUAL RESULT %d: Curl command exection failed. Details: %s" %(step, details))
                    print("[TEST EXECUTION RESULT] : SUCCESS")
                    ...
                    if trigger_flag != 1:
                        ...
                        if expectedresult in actualresult and details == "0":
                            tdkTestObj.setResultStatus("SUCCESS")
                            print("ACTUAL RESULT %d: Failed to add the Port Mapping profile or the added number of profiles are not expected. Device.NAT.PortMappingNumberOfEntries : %s" %(step, details))
                            print("[TEST EXECUTION RESULT] : SUCCESS")
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
                            print("ACTUAL RESULT %d: The Port Mapping profile is added. Device.NAT.PortMappingNumberOfEntries : %s" %(step, details))
        ...
        tdkTestObj.setResultStatus("FAILURE")
        print("ACTUAL RESULT %d : Failed to initiate Factory Reset. " %step)
        print("[TEST EXECUTION RESULT] : FAILURE")
