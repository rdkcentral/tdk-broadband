...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_WEBCONFIG_6G_WIFIValidationUsingPrivateSSIDSubdoc</name>
  <primitive_test_id/>
  <primitive_test_name>Webconfig_DoNothing</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Validate WIFI via Webconfig Feature using 6g privatessid subdoc</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WEBCONFIG_22</test_case_id>
    <test_objective>Validate WIFI via Webconfig Feature using 6g privatessid subdoc</test_objective>
...
Device.WiFi.AccessPoint.{i}.Security.X_COMCAST-COM_KeyPassphrase</input_parameters>
    ...
    4. Enter valid 2.4g privatessid subdoc information with new values in RDKM WebConfig Server via curl command.
    5. Set trigger parameter Device.X_RDK_WebConfig.ForceSync to root.
    6. Get the  WiFi parameters for 2.4GHz and validate whether it is updated.
    7. Unload the modules.</automation_approch>
    <expected_output>WIFI validation via Webconfig Feature using 6g privatessid subdoc should be successful.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WEBCONFIG</test_stub_interface>
    <test_script>TS_WEBCONFIG_6G_WIFIValidationUsingPrivateSSIDSubdoc</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WEBCONFIG_2G_WIFIValidationUsingPrivateSSIDSubdoc')
sysobj.configureTestCase(ip,port,'TS_WEBCONFIG_2G_WIFIValidationUsingPrivateSSIDSubdoc')
        ...
        if flag == 0:
            ...
            if expectedresult in actualresult and radio_entries in [2, 3]:
                ...
                if retrieval_flag != 1:
                    ...
                    subdoc_info = getSubdocInfo(subdoc_name, subdoc_type, WIFI_RADIO=wifi_radio, radio_entries=radio_entries, initial_param_values=initial_wifi_values)
                    ...
                    if expectedresult in actualresult and "Request successful" in details:
                        ...
                        if trigger_flag != 1:
                            ...
                            print("\nTEST STEP %d: Check whether the values of %s WiFi parameters are updated as per privatessid subdoc" %(step, wifi_radio))
                            print("EXPECTED RESULT %d: The values of WiFi parameters should be updated successfully" %step)
                            ...
                            if "FAILURE" not in result and final_values == set_values:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print("ACTUAL RESULT %d: The values of WiFi parameters have been updated as per privatessid subdoc successfully" %step)
                                print("[TEST EXECUTION RESULT] : SUCCESS")
                                ...
                                tdkTestObj.setResultStatus("FAILURE")
                                print("ACTUAL RESULT %d: Failed to update the WiFi parameters." %step)
                                print("[TEST EXECUTION RESULT] : FAILURE")
                        else:
