...
obj.configureTestCase(ip,port,'TS_Telemetry2_0_ValidateReportGeneration_ReportingIntervalExceedsTimeout')
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_ValidateReportGeneration_ReportingIntervalExceedsTimeout')
wifiobj.configureTestCase(ip,port,'TS_Telemetry2_0_ValidateReportGeneration_ReportingIntervalExceedsTimeout')
...
if expectedresult in loadmodulestatus.upper() and expectedresult in loadmodulestatus_sys.upper() and expectedresult in loadmodulestatus_wifi.upper():
    ...
    if getStatus == 1:
        ...
        if flag == 0:
            step += 1
            reportProfilesJSON = createReportProfilesJSON(numProfiles, profileType, scenario="invalid_timeout")
            ...
            print("Setting Report Profiles such that Reporting Interval exceeds Report Timeout")
            ...
            if check_flag == 1:
                ...
                if not profile_check:
                    ...
                    step += 1
                    print("\nTEST STEP %d: Check whether appropriate error logs are present in telemetry2.0 logs" %step)
                    print("EXPECTED RESULT %d: Appropriate error logs should be present in telemetry2.0 logs" %step)
                    cmd = f"cat /rdklogs/logs/telemetry2_0.txt.0 | grep {profile_names[0]} | grep 'invalid profile'"
                    print("Command : %s" %cmd)
                    tdkTestObj = sysobj.createTestStep('ExecuteCmd')
                    actualresult, details = doSysutilExecuteCommand(tdkTestObj, cmd)
                    print("Command Output : %s" %details)
                    if expectedresult in actualresult and 'activationTimeoutPeriod is less than reporting interval' in details:
                        tdkTestObj.setResultStatus("SUCCESS")
                        print("ACTUAL RESULT %d: Appropriate error logs are present in telemetry2.0 logs." %step)
                        print("[TEST EXECUTION RESULT] : SUCCESS")
                    else:
                        tdkTestObj.setResultStatus("FAILURE")
                        print("ACTUAL RESULT %d: Appropriate error logs are not present in telemetry2.0 logs." %step)
                        print("[TEST EXECUTION RESULT] : FAILURE")
                else:
                    tdkTestObj.setResultStatus("FAILURE")
