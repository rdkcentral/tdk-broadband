...
obj.configureTestCase(ip,port,'TS_Telemetry2_0_ReportProfile_InvalidMsgPack')
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_ReportProfile_InvalidMsgPack')
wifiobj.configureTestCase(ip,port,'TS_Telemetry2_0_ReportProfile_InvalidMsgPack')
...
if expectedresult in loadmodulestatus.upper() and expectedresult in loadmodulestatus_sys.upper() and expectedresult in loadmodulestatus_wifi.upper():
    ...
    if getStatus == 1:
        ...
        if flag == 0:
            step += 1
            #Create Report Profiles JSON body with Invalid MsgPack data
            reportProfilesJSON = createReportProfilesJSON(numProfiles, profileType, scenario="invalid_msgpack")
            profile_names = [profile["name"] for profile in reportProfilesJSON["profiles"]]
            msg_pack = JsontoMsgPackBase64(reportProfilesJSON)
            print("Report Profile MsgPack body to be set : %s" %msg_pack)

            print("Setting Report Profile with Invalid MsgPack Body")
            check_flag, initial_report_profiles, param_name, step = SetReportProfiles(wifiobj, msg_pack, profileType, -1, step)
            ...
            if check_flag == 1:
                ...
                step += 1
                print("\nTEST STEP %d: Check whether the profile is created in /nvram/.t2reportprofiles/ when invalid MsgPack is provided" %step)
                print("EXPECTED RESULT %d: Profile should not be created in /nvram/.t2reportprofiles/" %step)
