...
obj.configureTestCase(ip,port,'TS_Telemetry2_0_ValidateReportGeneration_InvalidReportingInterval')
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_ValidateReportGeneration_InvalidReportingInterval')
wifiobj.configureTestCase(ip,port,'TS_Telemetry2_0_ValidateReportGeneration_InvalidReportingInterval')
        ...
        if (t2_flag or t2_revert_flag) == 1 and rm_t2_flag:
            ...
            if check_flag == 1:
                ...
                if not profile_check:
                    ...
                    print("Command Ouput %s" %details)
                    if expectedresult in actualresult and 'invalid configuration' in details:
                        tdkTestObj.setResultStatus("SUCCESS")
                        print("ACTUAL RESULT %d: Invalid Reporting Interval logs are present in telemetry2.0 logs." %step)
                        print("[TEST EXECUTION RESULT] : SUCCESS")
                    else:
                        tdkTestObj.setResultStatus("FAILURE")
                        print("ACTUAL RESULT %d: Invalid Reporting Interval logs are not present in telemetry2.0 logs." %step)
                        print("[TEST EXECUTION RESULT] : FAILURE")
                else:
                    tdkTestObj.setResultStatus("FAILURE")
