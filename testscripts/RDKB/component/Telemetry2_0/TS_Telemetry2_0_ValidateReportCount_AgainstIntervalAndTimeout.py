...
obj.configureTestCase(ip,port,'TS_Telemetry2_0_ValidateReportCount_AgainstIntervalAndTimeout')
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_ValidateReportCount_AgainstIntervalAndTimeout')
wifiobj.configureTestCase(ip,port,'TS_Telemetry2_0_ValidateReportCount_AgainstIntervalAndTimeout')
            ...
            if check_flag == 1:
                ...
                if profile_check:
                    ...
                    if log_check:
                        ...
                        #Check whether the report frequency is as per the Reporting Interval and Activation Timeout
                        step += 1
                        ...
                        report_count = int(details.strip())
                        expected_count = ACTIVATION_TIMEOUT // REPORTING_INTERVAL
                        print("Number of reports generated in the logs : %d" %report_count)
                        print("Expected number of reports to be generated in the logs : %d" %expected_count)
                        if report_count == expected_count and log_check == True:
                            tdkTestObj.setResultStatus("SUCCESS")
                            print("ACTUAL RESULT %d: Report frequency is as per the Reporting Interval and Activation Timeout. Number of Reports : %d" % (step, report_count))
                            print("[TEST EXECUTION RESULT] : SUCCESS")
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
                            print("ACTUAL RESULT %d: Report frequency is not as per the Reporting Interval and Activation Timeout. Number of reports : %d" % (step, report_count))
                            print("[TEST EXECUTION RESULT] : FAILURE")
                    else:
                        tdkTestObj.setResultStatus("FAILURE")
                        print("ACTUAL RESULT %d: cJSON report is not generated in the telemetry2.0 logs. Profile List : %s" % (step, profile_names))
                        print("[TEST EXECUTION RESULT] : FAILURE")
