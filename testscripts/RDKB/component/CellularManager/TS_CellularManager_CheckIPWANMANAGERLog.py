...
<xml>
  ...
  <name>TS_CellularManager_CheckIPWANMANAGERLog</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Once Device.Cellular.X_RDK_Status shows connected, compare the ip listed in /rdklogs/logs/WANMANAGERLog.txt.0 and ifconfig wwan0.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_CellularManager_5</test_case_id>
    <test_objective>Once Device.Cellular.X_RDK_Status shows connected, compare the ip listed in /rdklogs/logs/WANMANAGERLog.txt.0 and ifconfig wwan0. Both the IPs should match.</test_objective>
...
3. Get IP address from /rdklogs/logs/WANMANAGERLog.txt.0
4. Get the wwan0 IP Address.
5. Compare both IP obtained and they must match for the test to succeed.
6. Unload the sysutil and tdkbtr181 module.</automation_approch>
    <expected_output>IP obtained from WANMANAGERLog.txt.0 and wwan0 IP should match.</expected_output>
    <priority>High</priority>
    <test_stub_interface>CellularManager_DoNothing</test_stub_interface>
    <test_script>TS_CellularManager_CheckIPWANMANAGERLog</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_CellularManager_CheckIPWANMANAGERLog');
sysobj.configureTestCase(ip,port,'TS_CellularManager_CheckIPWANMANAGERLog');
...
if "SUCCESS" in loadmodulestatus.upper() and loadmodulestatus_sys.upper():
    ...
    if expectedresult in actualresult and details == "CONNECTED":
        ...
        #Fetch IP address from WANMANAGERLog.txt.0
        tdkTestObj = sysobj.createTestStep('ExecuteCmd');
        tdkTestObj.addParameter("command", "cat /rdklogs/logs/WANMANAGERLog.txt.0 | grep -i \"assigned ip\" | awk '{print $6}' | cut -d'=' -f2");
        expectedresult="SUCCESS";
        #Execute the test case in STB
        tdkTestObj.executeTestCase(expectedresult);
        actualresult = tdkTestObj.getResult();
        details = tdkTestObj.getResultDetails();
        print("TEST STEP 2: Get the IP address from WANMANAGERLog.txt.0");
        print("EXPECTED RESULT 2: Should get the IP address from WANMANAGERLog.txt.0");
        ip_wanmanagerlog = "";

        if expectedresult in actualresult and details != "":
            ip_wanmanagerlog = details[:-2];
            print("ACTUAL RESULT 2: IP obtained from at /rdklogs/logs/WANMANAGERLog.txt.0 : %s" %ip_wanmanagerlog);
            ...
            print("ACTUAL RESULT 2:Error: Failed to obtain IP from at /rdklogs/logs/WANMANAGERLog.txt.0 ")
            #Set the result status of execution
        ...
        #Compare wwan0 IP and WANMANAGERLog.txt.0 IP
        print("TEST STEP 4: Compare wwan0 IP and WANMANAGERLog.txt.0 IP");
        print("EXPECTED RESULT 4: Both IPs must be the same");

        if ip_wanmanagerlog == ip_wwan0:
