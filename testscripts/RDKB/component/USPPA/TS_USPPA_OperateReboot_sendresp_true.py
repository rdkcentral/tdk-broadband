...
6. Unload sysutil module</automation_approch>
    <expected_output>The EUT reboots with a valid response and resumes connectivity with the test system successfully via USP protocol</expected_output>
    <priority>High</priority>
    <test_stub_interface>sysutil</test_stub_interface>
    <test_script>TS_USPPA_OperateReboot_sendresp_true</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_USPPA_OperateReboot_sendresp_true');
...
if "SUCCESS" in result.upper() :
    ...
    if "SUCCESS" in preRequisiteStatus:
        ...
        print("\nTEST STEP 1: Send OPERATE message to reboot the EUT with send_resp true and received valid response back via USP protocol")
        print("EXPECTED RESULT 1: Send OPERATE message to reboot the EUT with send_resp true successfully and received valid response back via USP protocol")
        queryParam = {"name":"Device.Reboot()", "send_resp":"true"}
        ...
        if status == 200 :
            ...
            if "SUCCESS" in preRequisiteStatus:
                ...
                print("ACTUAL RESULT 1: Sent OPERATE message to reboot the EUT with send_resp true successfully via USP protocol")
                #Parse the response from operate operation
                parsedResponse = parseUsppaResponse(queryResponse,"operate")
                if "SUCCESS" in parsedResponse[0] and parsedResponse[1]:
                    tdkTestObj.setResultStatus("SUCCESS")
                    print("Agent correctly processed the OPERATE message for reboot successfully via USP protocol.")
                    ...
                    tdkTestObj.setResultStatus("FAILURE")
                    print("Agent failed to process the OPERATE message for reboot via USP protocol.")
                    print("[TEST EXECUTION RESULT] : FAILURE")
            else:
                tdkTestObj.setResultStatus("FAILURE")
            ...
            tdkTestObj.setResultStatus("FAILURE")
            if status == "FAILURE":
                print(" Token Generation failed during operate operation")
            print(f"ACTUAL RESULT 1: Failed to send operate message to reboot the EUT with send_resp true via USP protocol with status: {status}")
            print("[TEST EXECUTION RESULT] : FAILURE")
    else:
        tdkTestObj.setResultStatus("FAILURE")
