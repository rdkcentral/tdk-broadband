...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send GET_INSTANCES request to get the instance details of controller instance with first_level_only true and receive a valid response via USP protocol.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>This test case is to send GET_INSTANCES request to get the instance details of controller instance with first_level_only true and receive a valid response via USP protocol.</test_objective>
...
if "SUCCESS" in result.upper() :
    ...
    if "SUCCESS" in preRequisiteStatus:
        ...
        print("\nTEST STEP 1: Send GET_INSTANCES request to get the instance details of controller instance with first_level_only true and receive a valid response via USP protocol.")
        print("EXPECTED RESULT 1: Send GET_INSTANCES request to get the instance details of controller instance with first_level_only true and receive a valid response successfully via USP protocol.")
        status,queryResponse = usppaQuery(agentID,queryParam ,"get_instances")
        if status == 200:
            tdkTestObj.setResultStatus("SUCCESS")
            print("ACTUAL RESULT 1: Sent GET_INSTANCES request to get the instance details of controller instance with first_level_only true successfully via USP protocol.")
