...
<xml>
  ...
  <name>TS_USPPA_GetSupportedDM_Object_firstlevelonly_true_noOptions</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To send a GET_SUPPORTED_DM request to retrieve the details of single object, with first_level_only set to true and options like return_params, return_commands and return_events are disabled via USP protocol</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_USPPA_29</test_case_id>
    <test_objective>This test case is to send a GET_SUPPORTED_DM request via the USP protocol to retrieve the supported data model details of the root object, with first_level_only set to true and options like return_params, return_commands and return_events are disabled.</test_objective>
...
4.USP agent and controller are up and communicating with each other.</pre_requisite>
    ...
    <input_parameters>Device.LocalAgent.</input_parameters>
    <automation_approch>1. Load sysutil module
2. Check the prerequisite function is success.
3. Configure USP controller to send GET_SUPPORTED_DM request for getting the details of object with first_level_only as true and options like return_params, return_commands and return_events are disabled.
4. Once request is success, parse the USP response and get the metadata alone of the object
5. Unload sysutil module</automation_approch>
    <expected_output>Should get the supported DM's metadata details of object with first_level_only as true and options like return_params, return_commands and return_events are disabled via USP protocol successfully.</expected_output>
    <priority>High</priority>
    <test_stub_interface>sysutil</test_stub_interface>
    <test_script>TS_USPPA_GetSupportedDM_Object_firstlevelonly_true_noOptions</test_script>
    <skipped>No</skipped>
