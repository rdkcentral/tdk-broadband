...
<xml>
  ...
  <name>TS_USPPA_AddDelete_ValidSubscription_allowpartial_false</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Send ADD message to add a valid agent subscription and DELETE message to delete the subscription with allow_partial as false and receive valid response via USP protocol</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_USPPA_11</test_case_id>
    <test_objective>This testcase is to send ADD message to add a valid agent subscription and DELETE message to delete the subscription with allow_partial as false and receive valid response via USP protocol</test_objective>
...
3. Configure USP controller to send ADD request for adding the valid Device.LocalAgent.Subscription. with allow_partial as false
...
6.Delete the subscription with allow_partial as false and parse the response to confirm deletion.
7. Unload sysutil module</automation_approch>
    ...
    <test_script>TS_USPPA_AddDelete_ValidSubscription_allowpartial_false</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_USPPA_AddDelete_ValidSubscription_allowpartial_false')
...
if "SUCCESS" in result.upper():
    ...
    if "SUCCESS" in preRequisiteStatus:
        ...
        print("\nTEST STEP 1: Send ADD message request to add a valid Agent subscription with allow_partial as false and receive valid response via USP protocol")
        print("EXPECTED RESULT 1: Send ADD message request to add a valid Agent subscription with allow_partial as false and receive valid response successfully via USP protocol")
        queryParam = {"name":"Device.LocalAgent.Subscription.","allow_partial":"false"}
        status,queryResponse = usppaQuery(agentID,queryParam,"add")
        if status == 200:
            tdkTestObj.setResultStatus("SUCCESS")
            print("ACTUAL RESULT 1: Sent ADD message request to add a valid Agent subscription with allow_partial as false successfully via USP protocol")
            ...
            if "SUCCESS" in parsedResponse[0] and parsedResponse[1]:
                ...
                tdkTestObj.setResultStatus("SUCCESS")
                print("ACTUAL RESULT 1: Agent correctly processed the ADD request to add a valid Agent Subscription successfully via USP protocol")
                ...
                if status == 200:
                    ...
                    print(f"ACTUAL RESULT 1: Failed to get the valid Agent Subscription via USP protocol with status: {status}")
                    print("[TEST EXECUTION RESULT] : FAILURE")
            else:
                tdkTestObj.setResultStatus("FAILURE")
                print("Agent failed to process the ADD request to add Agent Subscription via USP protocol")
                print("[TEST EXECUTION RESULT] : FAILURE")

            #Delete the newly added agent subscription
            print("\nTEST STEP 4: Send DELETE message request to delete newly added valid Agent subscription with allow_partial as false via USP protocol")
            print("EXPECTED RESULT 4: Send DELETE message request to delete newly added valid Agent subscription with allow_partial as false successfully via USP protocol")
            queryParam = {"name":f"Device.LocalAgent.Subscription.{instanceNumber}.","allow_partial":"false"}
            status,queryResponse = usppaQuery(agentID,queryParam,"delete")
            if status == 200:
                tdkTestObj.setResultStatus("SUCCESS")
                print("ACTUAL RESULT 4: Sent DELETE message request to delete newly added valid Agent subscription with allow_partial as false successfully via USP protocol")
                ...
                tdkTestObj.setResultStatus("FAILURE")
                print("ACTUAL RESULT 4: Failed to Send DELETE message to delete newly added valid Agent subscription with allow_partial as false via USP protocol with status: {status}")
                print("[TEST EXECUTION RESULT] : FAILURE")
        else:
            tdkTestObj.setResultStatus("FAILURE")
            if status == "FAILURE":
                print(" Token Generation failed during ADD operation")
            print(f"ACTUAL RESULT 1: Failed to add a valid Agent Subscription with allow_partial as false via USP protocol with status: {status}")
            print("[TEST EXECUTION RESULT] : FAILURE")
    else:
        tdkTestObj.setResultStatus("FAILURE")
