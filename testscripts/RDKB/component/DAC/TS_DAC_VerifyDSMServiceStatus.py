...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_DAC_VerifyDSMServiceStatus')
obj.configureTestCase(ip,port,'TS_DAC_VerifyDSMServiceStatus')
...
if "SUCCESS" in loadmodulestatus_sys.upper() and "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        step += 1
        service_name = DSM_SERVICE
        # Check dsm service status
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult and details == EXPECTED_ENABLED_STATE:
                ...
                step += 1
                # Verify dsm service is active (running)
                ...
                if expectedresult in actualresult and EXPECTED_ACTIVE_STATE in state:
                    ...
                    step += 1
                    # Verify dsm process is running
                    print("\nTEST STEP %d: Verify %s process is running" % (step, DSM_PROCESS))
                    print("EXPECTED RESULT %d: Process should be running with valid PID" % step)
                    tdkTestObj, actualresult, process_pid = verify_process_exists(sysobj, DSM_PROCESS)
                    if expectedresult in actualresult:
                        tdkTestObj.setResultStatus("SUCCESS")
                        print("ACTUAL RESULT %d: %s process is running with PID: %s" % (step, DSM_PROCESS, process_pid))
                        print("[TEST EXECUTION RESULT] : SUCCESS")
                    else:
                        tdkTestObj.setResultStatus("FAILURE")
                        print("ACTUAL RESULT %d: %s process is not running" % (step, DSM_PROCESS))
