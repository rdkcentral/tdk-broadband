...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To validate in RBUS mode, CcspTandDSsp process getting restarted after killed</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To validate in RBUS mode, CcspTandDSsp process getting restarted after killed</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband,RPI</test_setup>
    <pre_requisite>1.CcspTandDSsp  should be in a running state of DUT
...
4. Get the PID of CcspTandDSsp process and kill the process
5. Check whether the CcspTandDSsp process is restarted successfully
6. Execute post process of RBUS and it should be success
7. Unload the Modules</automation_approch>
    <expected_output>The CcspTandDSsp process should be restarted if its killed in RBUS mode</expected_output>
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if rbus_set == 1:
        ...
        print("******************************************************************")
        actualresult,pid_value = getPID(tdkTestObj_Sys_ExeCmd,"CcspTandDSsp");
        if expectedresult in actualresult and pid_value != "":
            tdkTestObj_Tr181_Get.setResultStatus("SUCCESS");
            print("TEST STEP 2: Get the PID value of CcspTandDSsp Process")
            print("EXPECTED RESULT 2: Should get the PID value of CcspTandDSsp process")
            ...
            if expectedresult in actualresult:
                tdkTestObj_Tr181_Get.setResultStatus("SUCCESS");
                print("TEST STEP 3: Kill the CcspTandDSsp Process")
                print("EXPECTED RESULT 3: CcspTandDSsp Process should be killed")
                print("ACTUAL RESULT 3: CcspTandDSsp process killed successfully")
                print("[TEST EXECUTION RESULT] 3: SUCCESS");
                print("******************************************************************")
                sleep(10);
                actualresult,pid_value = checkProcessRestarted(tdkTestObj_Sys_ExeCmd,"CcspTandDSsp");
                if expectedresult in actualresult and pid_value != "":
                    tdkTestObj_Tr181_Get.setResultStatus("SUCCESS");
                    print("TEST STEP 5: Get the PID value of CcspTandDSsp Process to make sure process is restarted")
                    print("EXPECTED RESULT 5: Should get the PID value of CcspTandDSsp process")
                ...
                print("TEST STEP 3: Kill the CcspTandDSsp Process")
                print("EXPECTED RESULT 3: CcspTandDSsp Process should be killed")
                print("ACTUAL RESULT 3: Fail to kill the CcspTandDSsp process")
