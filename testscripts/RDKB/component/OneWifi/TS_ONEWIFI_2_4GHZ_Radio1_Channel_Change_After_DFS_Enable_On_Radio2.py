...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        if "FAILURE" not in actualresult_all and dfs_rfc == "true" and dfs_enable == "true":
            ...
            # Step 3: Get current Radio 1 channel and PossibleChannels
            step += 1
            ...
            print(f"TEST STEP {step}: Get current Radio 1 (2.4 GHz) channel and PossibleChannels")
            print(f"EXPECTED RESULT {step}: Should get current channel and possible channels list for Radio 1")
            if expectedresult in actualresult_ch and expectedresult in actualresult_pc:
                ...
                # Step 4: Disable Auto Channel Selection on Radio 1
                step += 1
                tdkTestObj, actualresult = wifi_SetParam(obj, "Device.WiFi.Radio.1.AutoChannelEnable", "false", "boolean")
                sleep(2)
                print(f"TEST STEP {step}: Disable Auto Channel Selection on Radio 1")
                ...
                if expectedresult in actualresult:
                    ...
                    # Step 5: Change Radio 1 channel to a random channel from PossibleChannels
                    step += 1
                    tdkTestObj, actualresult = wifi_SetParam(obj, "Device.WiFi.Radio.1.Channel", target_channel, "unsignedint")
                    sleep(2)
                    print(f"TEST STEP {step}: Change Radio 1 channel to {target_channel}")
                    ...
                    if expectedresult in actualresult:
                        ...
                        # Step 6: Apply radio settings for Radio 1
                        step += 1
                        tdkTestObj, actualresult = wifi_SetParam(obj, "Device.WiFi.ApplyRadioSettings", "true", "boolean")
                        sleep(5)
                        print(f"TEST STEP {step}: Apply radio settings for Radio 1")
                        ...
                        # Step 7: Verify Radio 1 channel is updated
                        step += 1
                        tdkTestObj, actualresult, radio1_channel_after = wifi_GetParam(obj, "Device.WiFi.Radio.1.Channel")
                        print(f"TEST STEP {step}: Verify Radio 1 channel is updated to {target_channel}")
                ...
                tdkTestObj.setResultStatus("FAILURE")
                print(f"ACTUAL RESULT {step}: GET Radio 1 channel or PossibleChannels FAILURE")
                print("TEST EXECUTION RESULT : FAILURE")
        else:
            tdkTestObj.setResultStatus("FAILURE")
