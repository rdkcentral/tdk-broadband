...
<xml>
  ...
  <name>TS_ONEWIFI_5GHZ_WPA3_RFCEnabled_RADIUS_WPA_On_Open_Enterprise_Psk_SecurityModeTransition</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>When RFC WPA3-Personal-Transition is enabled check Radius and WPA configuration on security mode transition (None-&gt;WPA3-Enterprise-&gt;WPA3-Personal for 5GHZ</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_279</test_case_id>
    <test_objective>To check whether WPA3 Configuration will be reset or not on security mode transition(open-enterprise-psk) for 5GHZ</test_objective>
    ...
    Device.WiFi.AccessPoint.2.Security.RadiusSecret</input_parameters>
...
4. Check WPA and RADIUS config using
Device.WiFi.AccessPoint.2.Security.X_CISCO_COM_EncryptionMethod
Device.WiFi.AccessPoint.2.Security.PreSharedKey,
Device.WiFi.AccessPoint.2.Security.RadiusServerIPAddr,
Device.WiFi.AccessPoint.2.Security.RadiusServerPort
Device.WiFi.AccessPoint.2.Security.RadiusSecret.
The values should be empty, empty, 0.0.0.0, 0, empty respectively.
5. Change the security mode to WPA3-Enterprise ,Encryption method to AES and Radius serverIP, Serverport, RadiusSecret to new values.--done
6. Check if mode set is reflected in get and if the RADIUS configuration is set to the new values.--done
7. Now set to the enterprise mode WPA3-Personal encryption method to AES, Preshared key to new value.
...
10. Unload the module</automation_approch>
    <expected_output>When security mode changes for open to enterprise(None to WPA3-Enterprise) RADIUS config should set. When security mode changes from enterprise to psk (WPA3-Enterprise to WPA3-Personal ) RADIUS config should  reset.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WifiAgent</test_stub_interface>
    <test_script>TS_ONEWIFI_5GHZ_WPA3_RFCEnabled_RADIUS_WPA_On_Open_Enterprise_Psk_SecurityModeTransition</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_WPA3_RFCEnabled_RADIUS_WPA_On_Open_Enterprise_Psk_SecurityModeTransition')
sysobj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_WPA3_RFCEnabled_RADIUS_WPA_On_Open_Enterprise_Psk_SecurityModeTransition')
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if "FAILURE" not in actualresult_all:
        ...
        # Step 1: Get the initial security mode
        step = step + 1
        ...
        print("TEST STEP %s: Get the initial security mode" % step)
        print("EXPECTED RESULT %s: Should get the initial security mode" % step)

        if expectedresult in actualresult:
            tdkTestObj.setResultStatus("SUCCESS")
            print("ACTUAL RESULT %s: Initial security mode is %s" % (step,initialSecurityMode))
            print("[TEST EXECUTION RESULT] : SUCCESS")
            ...
            if pre_req_set == 1:
                ...
                # Step 2: Change security mode to None
                step = step + 1
                paramName = "Device.WiFi.AccessPoint.2.Security.ModeEnabled"
                ...
                if expectedresult in actualresult:
                    ...
                    # Step 3: Check Security mode and initial WPA config
                    step = step + 1
                    ...
                    print("TEST STEP %s: Check WPA and RADIUS config" % step)
                    print("EXPECTED RESULT %s: WPA and RADIUS config should be set when security mode is None" % step)

                    if "FAILURE" not in actualresult_all and actual_values == expected_values:
                        tdkTestObj.setResultStatus("SUCCESS")
                        print("ACTUAL RESULT %s: SecurityMode is '%s', Encryption method is '%s', PreSharedKey is '%s', RADIUS IP is '%s', RADIUS ServerPort is '%s', RADIUS Secret is '%s'" % (step, actual_values[0],actual_values[1],actual_values[5],actual_values[2],actual_values[3],actual_values[4]))
                        print("[TEST EXECUTION RESULT] : SUCCESS")

                        # Step 4: Change the security mode to WPA3-Enterprise, Encryption method to AES and Radius serverIP, Serverport, RadiusSecret to new values
                        step = step + 1
                        tdkTestObj = obj.createTestStep("WIFIAgent_SetMultiple")
                        paramList = ("Device.WiFi.AccessPoint.2.Security.ModeEnabled|WPA3-Enterprise|string|"
                        ...
                        sleep(1)
                        print("TEST STEP %s: Change security mode to WPA3-Enterprise, Encryption method to AES with new RADIUS config" % step)
                        print("EXPECTED RESULT %s: Should set security mode to WPA3-Enterprise, Encryption method to AES and RADIUS config parameters to new values" % step)

                        if expectedresult in actualresult:
                            tdkTestObj.setResultStatus("SUCCESS")
                            print("ACTUAL RESULT %s: Security mode set to WPA3-Enterprise, Encryption method to AES and RADIUS config updated" % step)
                            print("[TEST EXECUTION RESULT] : SUCCESS")

                            # Step 5: Check SecurityMode SET is reflected in get
                            step = step + 1
                            paramList_config = ["Device.WiFi.AccessPoint.2.Security.ModeEnabled",
                                    "Device.WiFi.AccessPoint.2.Security.X_CISCO_COM_EncryptionMethod",
                                    "Device.WiFi.AccessPoint.2.Security.RadiusServerIPAddr",
                            ...
                            expected_values = ["WPA3-Enterprise", ENCRYPTION_MODE, RADIUS_IP, RADIUS_PORT, ""]
                            actual_values = []
                            actualresult_all = []

                            for param in paramList_config:
                                tdkTestObj, actualresult, value = wifi_GetParam(obj, param)
                                actualresult_all.append
                                actual_values.append(value)

                            print("TEST STEP %s: Check security mode and RADIUS config" % step)
                            print("EXPECTED RESULT %s: Security mode should be WPA3-Enterprise with new RADIUS config" % step)

                            if "FAILURE" not in actualresult_all and actual_values == expected_values:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print("ACTUAL RESULT %s: Security mode is '%s', Encryption method is '%s', RADIUS IP is '%s', RADIUS ServerPort is '%s', RADIUSSecret is '%s'" % (step, actual_values[0],actual_values[1],actual_values[2],actual_values[3],actual_values[4]))
                                print("[TEST EXECUTION RESULT] : SUCCESS")

                                # Step 6: Set security mode to WPA3-Personal, encryption method to AES, Preshared key to new value
                                step = step +1
                                tdkTestObj = obj.createTestStep("WIFIAgent_SetMultiple")
                                ...
                                actualresult = tdkTestObj.getResult()
                                sleep(1)
                                print("TEST STEP %s: Set security mode to WPA3-Personal, encryption method to AES, Preshared key to new value" % step)
                                print("EXPECTED RESULT %s: Should set security mode to WPA3-Personal, encryption method to AES, Preshared key to new value" % step)

                                if expectedresult in actualresult:
                                    tdkTestObj.setResultStatus("SUCCESS")
                                    print("ACTUAL RESULT %s: Security mode set to WPA3-Personal, encryption method to AES, Preshared key to new value" % step)
                                    print("[TEST EXECUTION RESULT] : SUCCESS")

                                    # Step 7: Validate the change by performing a get operation
                                    step = step + 1
                                    paramList_config = ["Device.WiFi.AccessPoint.2.Security.ModeEnabled",
                                    "Device.WiFi.AccessPoint.2.Security.X_CISCO_COM_EncryptionMethod",
                                    "Device.WiFi.AccessPoint.2.Security.PreSharedKey",
                                    ...
                                    print("EXPECTED RESULT %s: Security mode should be WPA3-Personal, Encryption method should be AES, and KeyPassphrase  should remain the same" % step)
                                    ...
                                    if "FAILURE" not in actualresult_all and actual_values == expected_values:
                                        ...
                                        tdkTestObj.setResultStatus("FAILURE")
                                        print("ACTUAL RESULT %s: Validation failed. Security mode is '%s', encryption method is '%s', PreSharedKey is '%s',KeyPassphrase is '%s'" % (step, actual_values[0],actual_values[1],actual_values[2],actual_values[3]))
                                        print("[TEST EXECUTION RESULT] : FAILURE")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE")
                                    print("ACTUAL RESULT %s: Failed to set Security mode to WPA3-Personal, encryption method to AES, and Preshared key to new value" % step)
                                    print("[TEST EXECUTION RESULT] : FAILURE")
                            else:
                                tdkTestObj.setResultStatus("FAILURE")
                                print("ACTUAL RESULT %s: Security mode is '%s', Encryption method is '%s', RADIUS IP is '%s', RADIUS ServerPort is '%s', RADIUSSecret is '%s'" % (step, actual_values[0],actual_values[1],actual_values[2],actual_values[3],actual_values[4]))
                            ...
                            tdkTestObj.setResultStatus("FAILURE")
                            print("ACTUAL RESULT %s: Failed to set security mode to WPA3-Enterprise and Encryption method to AES with new configurations" % step)
                            print("[TEST EXECUTION RESULT] : FAILURE")
                    else:
                        tdkTestObj.setResultStatus("FAILURE")
                        print("ACTUAL RESULT %s: SecurityMode is '%s', Encryption method is '%s', PreSharedKey is '%s', RADIUS IP is '%s', RADIUS ServerPort is '%s', RADIUS Secret is '%s'" % (step, actual_values[0],actual_values[1],actual_values[5],actual_values[2],actual_values[3],actual_values[4]))
                    ...
                    tdkTestObj.setResultStatus("FAILURE")
                    print("ACTUAL RESULT %s: Failed to change security mode to None" % step)
                    print("[TEST EXECUTION RESULT] : FAILURE")
        ...
        tdkTestObj.setResultStatus("FAILURE")
        print("ACTUAL RESULT %d :Failed to get Radius and WPA config info from tdk_platform.properties" %step)
        print("[TEST EXECUTION RESULT] : FAILURE")

    # Step 8: Revert to initial security mode
    if revertflag == 1:
