...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>8</version>
  <name>TS_ONEWIFI_5GHzSetSAEPassPhrase_WithWPA3SecurityModes</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_179</test_case_id>
...
6. Set the Device.WiFi.AccessPoint.2.Security.ModeEnabled to WPA3-Personal and validate with get.
7. Check if the SET operation of Device.WiFi.AccessPoint.2.Security.SAEPassphrase to a valid new value returns success and validate with get.
8. Set the Device.WiFi.AccessPoint.2.Security.ModeEnabled to WPA3-Personal-Transition and validate with get.
9. Check if the SET operation of Device.WiFi.AccessPoint.2.Security.SAEPassphrase to a valid new value returns success and validate with get.
10. Revert the WPA3 RFC Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.WPA3_Personal_Transition.Enable status if required.
11. Revert the security mode enabled and the security configuration values to initial values if required.
12. Unload the modules.</automation_approch>
    ...
    <test_script>TS_ONEWIFI_5GHzSetSAEPassPhrase_WithWPA3SecurityModes</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_5GHzSetSAEPassPhrase_WithWPA3SecurityModes');
            ...
            if pre_req_set == 1:
                ...
                for mode in wpa3_modes:
                    ...
                    #Set Security Key/SAE Passphrase and Encryption Method if initial mode is not personal
                        ...
                        print("New KeyPassphrase and Encryption method to be set : %s" %new_passphrase_config);
                ...
                if (revert_flag == 0 and initial_mode != final_mode) or (revert_flag == 1 and initial_mode != "WPA2-Personal"):
                    ...
                    step = step + 1
                    tdkTestObj, actualresult, final_mode = wifi_GetParam(obj, "Device.WiFi.AccessPoint.2.Security.ModeEnabled")

                    print("\nTEST STEP %d : Check if Device.WiFi.AccessPoint.2.Security.ModeEnabled fell back to WPA2-Personal" %step);
                    print("EXPECTED RESULT %d : Device.WiFi.AccessPoint.2.Security.ModeEnabled should fall back to WPA2-Personal after disabling WPA3 Transition enable RFC" %step);
