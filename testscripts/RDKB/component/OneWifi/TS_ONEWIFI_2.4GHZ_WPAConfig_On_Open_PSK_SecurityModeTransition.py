...
<xml>
  ...
  <name>TS_ONEWIFI_2.4GHZ_WPAConfig_On_Open_PSK_SecurityModeTransition</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify WPA Configuration on transition of security modes form open to psk for 2.4ghz</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_261</test_case_id>
    <test_objective>To check whether WPA Configuration will be reset or not on security mode transition(open-psk-psk-psk) for 2.4ghz</test_objective>
...
4. Check initial WPA config using Device.WiFi.AccessPoint.1.Security.X_CISCO_COM_EncryptionMethod and Device.WiFi.AccessPoint.1.Security.PreSharedKey. Both the values should be empty when security mode is "None".
...
7. Then set to the next personal mode WPA2-Personal by setting Device.WiFi.AccessPoint.1.Security.ModeEnabled.
8. Check if mode set is reflected in get and check if the WPA configuration is not reset.
9. Then set to the next personal mode WPA-WPA2-Personal by setting Device.WiFi.AccessPoint.1.Security.ModeEnabled.
...
12. Unload the module</automation_approch>
    ...
    <test_script>TS_ONEWIFI_2.4GHZ_WPAConfig_On_Open_PSK_SecurityModeTransition</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_2.4GHZ_WPAConfig_On_Open_PSK_SecurityModeTransition')
sysobj.configureTestCase(ip,port,'TS_ONEWIFI_2.4GHZ_WPAConfig_On_Open_PSK_SecurityModeTransition')
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult:
                ...
                if "FAILURE" not in actualresult_all and actual_values[0] == "None" and actual_values[1] == "" and actual_values[2] == "" and actual_values[3] == "0.0":
                    ...
                    paramList = ("Device.WiFi.AccessPoint.1.Security.ModeEnabled|WPA-Personal|string|Device.WiFi.AccessPoint.1.Security.X_CISCO_COM_EncryptionMethod|%s|string|Device.WiFi.AccessPoint.1.Security.PreSharedKey|%s|string" %(ENCRYPTION_MODE,PRESHAREDKEY))
