...
<xml>
  ...
  <name>TS_ONEWIFI_5GHZ_WPAmode_RADIUS_WPA_On_Open_Psk_Enterprise_SecurityModeTransition</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check RADIUS and WPA configurations on security mode transition(None->WPA-Personal->WPA-Enterprise) for 5ghz</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_273</test_case_id>
    <test_objective>To check whether RADIUS and WPA configuration will be reset or not on security mode transition(None->WPA-Personal->WPA-Enterprise) for 5ghz</test_objective>
...
4. Check WPA and RADIUS config using Device.WiFi.AccessPoint.2.Security.X_CISCO_COM_EncryptionMethod Device.WiFi.AccessPoint.2.Security.PreSharedKey, Device.WiFi.AccessPoint.2.Security.RadiusServerIPAddr, Device.WiFi.AccessPoint.2.Security.RadiusServerPort and Device.WiFi.AccessPoint.2.Security.RadiusSecret. The values should be empty, empty, 0.0.0.0, 0, empty respectively.
5. Change the security mode to WPA-Personal ,Encryption method to AES and Preshared key to a new value.
6. Check if mode set is reflected in get and if the WPA configuration is set to the new values.
7. Now set to the enterprise mode WPA-Enterprise encryption method to AES, radius parameters: radius server, radius secret, radius port to valid values.
8. Check if security mode set is reflected in get and if the RADIUS configuration is set to the new values. Also check if the WPA configuration Preshared Key returns empty.
9. Revert to initial security mode.
10.unload the module</automation_approch>
    <expected_output>When security mode changes for open to enterprise(None to WPA-Enterprise) RADIUS config should set. When security mode changes from enterprise to psk (WPA-Enterprise to WPA-Personal ) RADIUS config should  reset.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WifiAgent</test_stub_interface>
    <test_script>TS_ONEWIFI_5GHZ_WPAmode_RADIUS_WPA_On_Open_Psk_Enterprise_SecurityModeTransition</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_WPAmode_RADIUS_WPA_On_Open_Psk_Enterprise_SecurityModeTransition')
sysobj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_WPAmode_RADIUS_WPA_On_Open_Psk_Enterprise_SecurityModeTransition')
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if "FAILURE" not in actualresult_all:
        ...
        if expectedresult in actualresult:
            ...
            # Step 3: Change security mode to None
            paramName = "Device.WiFi.AccessPoint.2.Security.ModeEnabled"
            ...
            if expectedresult in actualresult:
                ...
                if "FAILURE" not in actualresult_all and expected_values == actual_values:
                    ...
                    #Change security mode to WPA-Personal, encryption method to AES and PresharedKey to Aa12345678
                    tdkTestObj = obj.createTestStep("WIFIAgent_SetMultiple")
                    paramList = ("Device.WiFi.AccessPoint.2.Security.ModeEnabled|WPA-Personal|string|"
                    ...
                    print("\nTEST STEP 6: Change security mode to WPA-Personal, Encryption mode to AES and PresharedKey to 'Aa12345678'")
                    print("EXPECTED RESULT 6: Should set security mode to WPA-Personal and WPA config parameters to new values")

                    if expectedresult in actualresult:
                        tdkTestObj.setResultStatus("SUCCESS")
                        print("ACTUAL RESULT 6: Security mode set to WPA-Personal, WPA config parameters are updated")
                        print("[TEST EXECUTION RESULT] : SUCCESS")
                        ...
                        expected_values = ["WPA-Personal", "AES", "", "Aa12345678"]
                        actual_values = []
                        actualresult_all = []
                        ...
                        print("\nTEST STEP 7: Check security mode and WPA configuration Values")
                        print(f"EXPECTED RESULT 7: Security mode should be WPA-Personal Encryption mode should be {ENCRYPTION_MODE} and KeyPassphrase should be 'Aa12345678'")

                        if expectedresult in actualresult and actual_values == expected_values:
                            tdkTestObj.setResultStatus("SUCCESS")
                            ...
                            #Set security mode to WPA-Enterprise and RADIUS config parameters to new values
                            ...
                            actualresult = tdkTestObj.getResult()
                            sleep(2)
                            print("\nTEST STEP 8: Set security mode to WPA-Enterprise and RADIUS configuration to new values")
                            print(f"EXPECTED RESULT 8: Should set security mode to WPA-Enterprise Encryption mode to {ENCRYPTION_MODE}, RADIUSserverIP to {RADIUS_IP}, RADIUSserverPort to {RADIUS_PORT} and RADIUSsecret to {RADIUS_SECRET}")

                            if expectedresult in actualresult:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print("ACTUAL RESULT 8: Security mode set to WPA-Enterprise and RADIUS configuration with new values")
                                print("[TEST EXECUTION RESULT] : SUCCESS")
                                ...
                                expected_values = ["WPA-Enterprise", ENCRYPTION_MODE, RADIUS_IP, RADIUS_PORT, "",""]
                                actual_values = []
                                actualresult_all = []
                                ...
                                print("\nTEST STEP 9: Check SecurityMode, RADIUS Config and PreSharedKey")
                                print("EXPECTED RESULT 9: Security mode should be WPA-Enterprise, RADIUS config parameters with new values and PreSharedKey should be empty")

                                if "FAILURE" not in actualresult_all and actual_values == expected_values:
                                    tdkTestObj.setResultStatus("SUCCESS")
                                    print("ACTUAL RESULT 9: Security mode is '%s',  Encryption method is ' %s', RADIUS IP is ' %s', RADIUS ServerPort is ' %s', RADIUSSecret is ' %s' , PreSharedKey is ' %s'" % (actual_values[0],actual_values[1],actual_values[2],actual_values[3],actual_values[4],actual_values[5]))
                                    print("[TEST EXECUTION RESULT] : SUCCESS")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE")
                                    print("ACTUAL RESULT 9: Security mode is '%s',  Encryption method is ' %s', RADIUS IP is ' %s', RADIUS ServerPort is ' %s', RADIUSSecret is ' %s' , PreSharedKey is ' %s'" % (actual_values[0],actual_values[1],actual_values[2],actual_values[3],actual_values[4],actual_values[5]))
                                ...
                                tdkTestObj.setResultStatus("FAILURE")
                                print("ACTUAL RESULT 8: Failed Security mode set to WPA-Enterprise and RADIUS configuration to new values")
                                print("[TEST EXECUTION RESULT] : FAILURE")
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
                            print("ACTUAL RESULT 7: Security mode is '%s', Encryption method is '%s', PreSharedKey is ' %s', KeyPassphrase is ' %s'" % (actual_values[0],actual_values[1],actual_values[2],actual_values[3]))
                        ...
                        tdkTestObj.setResultStatus("FAILURE")
                        print("ACTUAL RESULT 6: Failed to set security mode set to WPA-Personal and WPA config to new values")
                        print("[TEST EXECUTION RESULT] : FAILURE")
                else:
                    tdkTestObj.setResultStatus("FAILURE")
                    print("ACTUAL RESULT 5: SecurityMode is '%s',Encryption Mode is ' %s', PresharedKey is ' %s', KeyPasspharse is ' %s', RADIUS IP is ' %s', RADIUS ServerPort is ' %s', RADIUS Secret is ' %s'" % (actual_values[0],actual_values[1],actual_values[5],actual_values[6],actual_values[2],actual_values[3],actual_values[4]))
