...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_DFS_RFC_Disable_Transitions_DFSEnable_False')
    ...
    if expectedresult in actualresult:
        ...
        if "FAILURE" not in actualresult_all and dfs_rfc == "true" and dfs_enable == "true":
            ...
            if expectedresult in actualresult:
                ...
                # Step 4: Verify both DMs are now false
                step += 1
                paramResults = {}
                actualresult_all = []
                for paramName in paramNames:
                ...
                tdkTestObj.setResultStatus("FAILURE")
                print(f"ACTUAL RESULT {step}: Disable DFS RFC SET FAILURE")
                print("TEST EXECUTION RESULT : FAILURE")
        else:
            tdkTestObj.setResultStatus("FAILURE")
