...
<xml>
  ...
  <name>TS_ONEWIFI_CheckTelemetryMarker_6GHZ_RETRYCOUNT_MULTIPLERETRYCOUNT</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if the Retry and Multiple Retry count via TR-181 and telemetry marker are greater than or equal to  zero by changing the log interval to 5 min</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_304</test_case_id>
...
Device.WiFi.AccessPoint.17.AssociatedDevice.1.Stats.RetryCount
Device.WiFi.AccessPoint.17.AssociatedDevice.1.Stats.MultipleRetryCount</input_parameters>
...
4.Check for telemetry marker WIFI_RETRYCOUNT_17 in wifihealth.txt and get the bytes sent using Device.WiFi.AccessPoint.17.AssociatedDevice.1.Stats.RetryCount
5.Check if the values received are greater than or equal to zero
6.Check for telemetry marker WIFI_MULTIPLERETRYCOUNT_17 in wifihealth.txt and get the bytes sent using Device.WiFi.AccessPoint.17.AssociatedDevice.1.Stats.MultipleRetryCount
...
9.Unload the Module</automation_approch>
    ...
    <test_script>TS_ONEWIFI_CheckTelemetryMarker_6GHZ_RETRYCOUNT_MULTIPLERETRYCOUNT</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_CheckTelemetryMarker_6GHZ_RETRYCOUNT_MULTIPLERETRYCOUNT')
sysObj.configureTestCase(ip,port,'TS_ONEWIFI_CheckTelemetryMarker_6GHZ_RETRYCOUNT_MULTIPLERETRYCOUNT')
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in actualresult and tel_loginterval == "300":
                        ...
                        if expectedresult in actualresult and whl_found == 1:
                            ...
                            #Check for WIFI_RETRYCOUNT_17 Marker in wifihealth log file
                            step += 1
                            print(f"TEST STEP {step}: Check if WIFI_RETRYCOUNT_17 Marker is present in wifihealth log file")
                            print(f"EXPECTED RESULT {step}: WIFI_RETRYCOUNT_17 Marker should be present wifihealth log file")
                            markerfound = 0
                            for i in range(1, 6):
                                if markerfound == 1:
                                    break
                                else:
                                    print(f"Checking WIFI_RETRYCOUNT_17 in wifihealth log file iteration {i}")
                                    query = "cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_RETRYCOUNT_17:\""
                                    tdkTestObj = sysObj.createTestStep('ExecuteCmd')
                                    tdkTestObj.addParameter("command", query)
                                    tdkTestObj.executeTestCase(expectedresult)
                                    actualresult = tdkTestObj.getResult()
                                    details = tdkTestObj.getResultDetails()
                                    if (len(details) == 0) or details.endswith(":") or "WIFI_RETRYCOUNT_17" not in details:
                                        markerfound = 0
                                        sleep(60)
                                    else:
                                        tel_retrycount = details.split("WIFI_RETRYCOUNT_17:")[1].split(',')[0].replace("\\n", "").strip()
                                        markerfound = 1

                            if expectedresult in actualresult and markerfound == 1 and tel_retrycount != "":
                                tdkTestObj.setResultStatus("SUCCESS")
                                print(f"ACTUAL RESULT {step}: WIFI_RETRYCOUNT_17 Marker is {tel_retrycount}")
                                print("[TEST EXECUTION RESULT] : SUCCESS")

                                # Bytes Sent via TR-181
                                step += 1
                                tdkTestObj = obj.createTestStep('pam_GetParameterValues')
                                tdkTestObj.addParameter("ParamName", "Device.WiFi.AccessPoint.17.AssociatedDevice.1.Stats.RetryCount")
                                tdkTestObj.executeTestCase(expectedresult)
                                actualresult = tdkTestObj.getResult()
                                tr181_retrycount = tdkTestObj.getResultDetails()

                                print(f"TEST STEP {step}: Get the RETRYCOUNT via TR-181 parameter")
                                print(f"EXPECTED RESULT {step}: Should get the RETRYCOUNT via TR-181 parameter")

                                if expectedresult in actualresult:
                                    tdkTestObj.setResultStatus("SUCCESS")
                                    print(f"ACTUAL RESULT {step}: RETRYCOUNT via TR-181 parameter: {tr181_retrycount}")
                                    print("[TEST EXECUTION RESULT] : SUCCESS")

                                    # Verify RETRYCOUNT >= 0
                                    step += 1
                                    print(f"TEST STEP {step}: Check if RETRYCOUNT via TR-181 parameter and telemetry marker are greater than or equal tozero")
                                    print(f"EXPECTED RESULT {step}: RETRYCOUNT via TR-181 parameter and telemetry marker should be greater than or equal to zero")
                                    if int(tel_retrycount) >= 0 and int(tr181_retrycount) >= 0:
                                        tdkTestObj.setResultStatus("SUCCESS")
                                        print(f"ACTUAL RESULT {step}:  RETRYCOUNT via TR-181: {tr181_retrycount}, RETRYCOUNT via Telemetry marker: {tel_retrycount}")
                                        print("[TEST EXECUTION RESULT] : SUCCESS")

                                        # Check WIFI_MULTIPLERETRYCOUNT_17 Marker in wifihealth log
                                        step += 1
                                        print(f"TEST STEP {step}: Check if WIFI_MULTIPLERETRYCOUNT_17 Marker is present in wifihealth log")
                                        print(f"EXPECTED RESULT {step}: WIFI_MULTIPLERETRYCOUNT_17 Marker should be present in wifihealth log")
                                        markerfound = 0
                                        for i in range(1, 6):
                                            if markerfound == 1:
                                                break
                                            else:
                                                print(f"Checking WIFI_MULTIPLERETRYCOUNT_17 in wifihealth log file iteration {i}")
                                                query = "cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_MULTIPLERETRYCOUNT_17:\""
                                                tdkTestObj = sysObj.createTestStep('ExecuteCmd')
                                                tdkTestObj.addParameter("command", query)
                                                expectedresult = "SUCCESS"
                                                tdkTestObj.executeTestCase(expectedresult)
                                                actualresult = tdkTestObj.getResult()
                                                details = tdkTestObj.getResultDetails()
                                                if (len(details) == 0) or details.endswith(":") or "WIFI_MULTIPLERETRYCOUNT_17" not in details:
                                                    markerfound = 0
                                                    sleep(60)
                                                else:
                                                    tel_mulretrycount = details.split("WIFI_MULTIPLERETRYCOUNT_17:")[1].split(',')[0].replace("\\n", "").strip()
                                                    markerfound = 1
                                        if expectedresult in actualresult and markerfound == 1 and tel_mulretrycount != "":
                                            tdkTestObj.setResultStatus("SUCCESS")
                                            print(f"ACTUAL RESULT {step}: WIFI_MULTIPLERETRYCOUNT_17 Marker is {tel_mulretrycount}")
                                            print("[TEST EXECUTION RESULT] : SUCCESS")
                                            # Bytes Received via TR-181
                                            step += 1
                                            tdkTestObj = obj.createTestStep('pam_GetParameterValues')
                                            tdkTestObj.addParameter("ParamName", "Device.WiFi.AccessPoint.17.AssociatedDevice.1.Stats.MultipleRetryCount")
                                            tdkTestObj.executeTestCase(expectedresult)
                                            actualresult = tdkTestObj.getResult()
                                            tr181_mulretrycount = tdkTestObj.getResultDetails()

                                            print(f"TEST STEP {step}: Get the  MULTIPLERETRYCOUNT via TR-181 parameter")
                                            print(f"EXPECTED RESULT {step}: Should get the  MULTIPLERETRYCOUNT via TR-181 parameter")

                                            if expectedresult in actualresult:
                                                tdkTestObj.setResultStatus("SUCCESS")
                                                print(f"ACTUAL RESULT {step}: MULTIPLERETRYCOUNT via TR-181 parameter: {tr181_mulretrycount}")
                                                print("[TEST EXECUTION RESULT] : SUCCESS")

                                                # Verify MULTIPLERETRYCOUNT >= 0
                                                step += 1
                                                print(f"TEST STEP {step}: Check if MULTIPLERETRYCOUNT via TR-181 parameter and telemetry marker are greater than zero")
                                                print(f"EXPECTED RESULT {step}: MULTIPLERETRYCOUNT via TR-181 parameter and telemetry marker should be greater than zero")
                                                if int(tel_mulretrycount) >= 0 and int(tr181_mulretrycount) >= 0:
                                                    tdkTestObj.setResultStatus("SUCCESS")
                                                    print(f"ACTUAL RESULT {step}: MULTIPLERETRYCOUNT via TR-181: {tr181_mulretrycount},MULTIPLERETRYCOUNT via Telemetry marker: {tel_mulretrycount}")
                                                    print("[TEST EXECUTION RESULT] : SUCCESS")
                                                else:
                                                    tdkTestObj.setResultStatus("FAILURE")
                                                    print(f"ACTUAL RESULT {step}: MULTIPLERETRYCOUNT via TR-181: {tr181_mulretrycount}, MULTIPLERETRYCOUNT via Telemetry marker: {tel_mulretrycount}")
                                            ...
                                            tdkTestObj.setResultStatus("FAILURE")
                                            print(f"ACTUAL RESULT {step}: WIFI_MULTIPLERETRYCOUNT_17 Marker not present")
                                            print("[TEST EXECUTION RESULT] : FAILURE")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE")
                                        print(f"ACTUAL RESULT {step}: RETRYCOUNT via TR-181: {tr181_retrycount}, RETRYCOUNT via Telemetry marker: {tel_retrycount}")
                                ...
                                tdkTestObj.setResultStatus("FAILURE")
                                print(f"ACTUAL RESULT {step}: WIFI_RETRYCOUNT_17 Marker not found in wifihealth log file")
                                print("[TEST EXECUTION RESULT] : FAILURE")
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
