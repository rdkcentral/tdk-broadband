...
<xml>
  ...
  <name>TS_ONEWIFI_5GHZ_WPA2_RADIUS_WPA_On_Open_Psk_Enterprise_SecurityModeTransition</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check RADIUS and WPA configurations on security mode transition(None->WPA2-Personal->WPA2-Enterprise) for 5ghz</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_267</test_case_id>
    <test_objective>To check whether RADIUS and WPA configuration will be reset or not on security mode transition(None->WPA2-Personal->WPA2-Enterprise) for 5ghz</test_objective>
    <test_type></test_type>
    ...
    5. Change the security mode to WPA2-Personal ,Encryption method to AES and Preshared key to a new value.
    6. Check if mode set is reflected in get and if the WPA configuration is set to the new values.
    7. Now set to the enterprise mode WPA2-Enterprise encryption method to AES, radius parameters: radius server, radius secret, radius port to valid values.
    ...
    <expected_output>When security mode changes form open to psk (None to WPA2-Personal) WPA config should set and When security mode changes from psk to enterprise(WPA2-Personal to WPA2-Enterprise) WPA config should reset</expected_output>
    <priority>High</priority>
    <test_stub_interface>WifiAgent</test_stub_interface>
    <test_script>TS_ONEWIFI_5GHZ_WPA2_RADIUS_WPA_On_Open_Psk_Enterprise_SecurityModeTransition</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_WPA2_RADIUS_WPA_On_Open_Psk_Enterprise_SecurityModeTransition')
sysobj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_WPA2_RADIUS_WPA_On_Open_Psk_Enterprise_SecurityModeTransition')
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult:
                ...
                if "FAILURE" not in actualresult_all and actual_values == expected_values:
                    ...
                    #Change security mode to WPA2-Personal, encryption method to AES and PresharedKey to Aa12345678
                    tdkTestObj = obj.createTestStep("WIFIAgent_SetMultiple")
                    paramList = ("Device.WiFi.AccessPoint.2.Security.ModeEnabled|WPA2-Personal|string|"
                    ...
                    print(f"TEST STEP 6: Change security mode to WPA2-Personal, Encryption mode to{ENCRYPTION_MODE}  and PresharedKey to {PRESHAREDKEY} ")
                    print("EXPECTED RESULT 6: Should set security mode to WPA2-Personal and WPA config parameters to new values")

                    if expectedresult in actualresult:
                        tdkTestObj.setResultStatus("SUCCESS")
                        print("ACTUAL RESULT 6: Security mode set to WPA2-Personal, WPA config parameters are updated")
                        print("[TEST EXECUTION RESULT] : SUCCESS")
                        ...
                        print("TEST STEP 7: Check security mode and WPA configuration Values")
                        print(f"EXPECTED RESULT 7: Security mode should be WPA2-Personal Encryption mode should be {ENCRYPTION_MODE} and KeyPassphrase should be 'Aa12345678'")
                        ...
                        if "FAILURE" not in actualresult_all and actual_values == expected_values:
                            ...
                            #Set security mode to WPA2-Enterprise and RADIUS config parameters to new values

                            tdkTestObj = obj.createTestStep("WIFIAgent_SetMultiple")
                            paramList = ("Device.WiFi.AccessPoint.2.Security.ModeEnabled|WPA2-Enterprise|string|"
                                "Device.WiFi.AccessPoint.2.Security.X_CISCO_COM_EncryptionMethod|%s|string|"
                            ...
                            print("TEST STEP 8: Set security mode to WPA2-Enterprise and RADIUS configuration to new values")
                            print(f"EXPECTED RESULT 8: Should set security mode to WPA2-Enterprise Encryption mode to {ENCRYPTION_MODE}, RADIUSserverIP to {RADIUS_IP}, RADIUSserverPort to {RADIUS_PORT} and RADIUSsecret to {RADIUS_SECRET}")

                            if expectedresult in actualresult:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print("ACTUAL RESULT 8: Security mode set to WPA2-Enterprise and RADIUS configuration with new values")
                                print("[TEST EXECUTION RESULT] : SUCCESS")
                                ...
                                expected_values = ["WPA2-Enterprise", ENCRYPTION_MODE, RADIUS_IP, RADIUS_PORT, "",""]
                                actual_values = []
                                actualresult_all = []
                                ...
                                for param in paramList_config:
                                    ...
                                    actualresult_all.append(actualresult)
                                    actual_values.append(value)
                                print("TEST STEP 9: Check SecurityMode, RADIUS Config and PreSharedKey")
                                print("EXPECTED RESULT 9: Security mode should be WPA2-Enterprise, RADIUS config parameters with new values and PreSharedKey should be empty")
