...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_CheckTelemetryMarker_ErrorSent')
sysObj.configureTestCase(ip,port,'TS_ONEWIFI_CheckTelemetryMarker_ErrorSent')
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in actualresult and tel_loginterval == "300":
                        ...
                        if expectedresult in actualresult and whl_found == 1:
                            ...
                            # Check for any WIFI_ERRORSSENT marker in wifihealth log file
                            step += 1
                            print(f"TEST STEP {step}: Check if WIFI_ERRORSSENT telemetry marker is present in wifihealth log file")
                            print(f"EXPECTED RESULT {step}: WIFI_ERRORSSENT telemetry marker should be present and value should be retrieved successfully")
                            ...
                            for i in range(1, 6):
                                if markerfound == 1:
                                    ...
                                    print(f"Checking WIFI_ERRORSSENT marker in wifihealth log file iteration {i}")
                                    query = "cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_ERRORSSENT_\" | grep -vi \"TOTAL_COUNT\" | tail -1"
                                    tdkTestObj = sysObj.createTestStep('ExecuteCmd')
                                    ...
                                    if len(details) == 0 or "WIFI_ERRORSSENT_" not in details:
                                        markerfound = 0
                                        sleep(60)
                                    else:
                                        marker_match = re.search(r"(WIFI_ERRORSSENT_[0-9]+):\s*([0-9]+)", details, re.IGNORECASE)

                                        if marker_match:
                                            marker_name = marker_match.group(1)
                                            tel_errorsent = marker_match.group(2).strip()
                                            markerfound = 1
                                        else:
                                            markerfound = 0
                                            sleep(60)
                            ...
                            if expectedresult in actualresult and markerfound == 1 and tel_errorsent != "":
                                tdkTestObj.setResultStatus("SUCCESS")
                                print(f"ACTUAL RESULT {step}: {marker_name} Marker is {tel_errorsent}")
                                print("[TEST EXECUTION RESULT] : SUCCESS")

                                step += 1
                                print(f"TEST STEP {step}: Check if ERRORSENT telemetry marker value is greater than zero")
                                print(f"EXPECTED RESULT {step}: ERRORSENT telemetry marker value should be greater than zero")

                                if int(tel_errorsent) > 0:
                                    tdkTestObj.setResultStatus("SUCCESS")
                                    print(f"ACTUAL RESULT {step}: ERRORSENT telemetry marker value: {tel_errorsent}")
                                    print("[TEST EXECUTION RESULT] : SUCCESS")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE")
                                    print(f"ACTUAL RESULT {step}: ERRORSENT telemetry marker value: {tel_errorsent}")
                                ...
                                tdkTestObj.setResultStatus("FAILURE")
                                print(f"ACTUAL RESULT {step}: WIFI_ERRORSSENT telemetry marker not found in wifihealth log file")
                                print("[TEST EXECUTION RESULT] : FAILURE")
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
