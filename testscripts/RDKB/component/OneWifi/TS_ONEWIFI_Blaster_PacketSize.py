...
<xml>
  ...
  <name>TS_ONEWIFI_Blaster_PacketSize</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To validate set operation of PacketSize parameter by passing valid and invalid values</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    ...
    <input_parameters> Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.WifiClient.ActiveMeasurements.Enable, Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.WifiClient.ActiveMeasurements.PacketSize</input_parameters>
    <automation_approch>1. Load wifiagent module
2. Set the ActiveMeasurements Enable parameter value to true
3. Set valid values ["64", "1100", "1470"] to PacketSize parameter and verify if set operation is successful
4. Set invalid values ["63", "1471"] to PacketSize parameter and verify if set operation fails
5. Unload wifiagent module</automation_approch>
    <expected_output>PacketSize should set valid values and fail setting invalid values</expected_output>
    <priority>High</priority>
    <test_stub_interface>ONEWIFI</test_stub_interface>
    <test_script>TS_ONEWIFI_Blaster_PacketSize</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_Blaster_PacketSize');
            ...
            if expectedresult in actualresult:
                ...
                for sample in valid_list:
                    ...
                    if expectedresult in actualresult:
                        ...
                        print("PacketSize successfully set to %s" %sample);
                    else:
                        set_success.append(0);
                        print("PacketSize failed to set %s" %sample);
                ...
                for invalid_sample in invalid_list:
                    ...
                    if expectedresult in actualresult:
                        ...
                        print("PacketSize failed to set  %s" %invalid_sample);
                    else:
                        set_failure.append(0);
                        print("PacketSize successfully set for some invalid values %s" %invalid_sample);
