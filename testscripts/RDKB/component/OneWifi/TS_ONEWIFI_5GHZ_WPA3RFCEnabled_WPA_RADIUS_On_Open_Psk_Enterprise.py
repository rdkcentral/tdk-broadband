...
<xml>
  ...
  <name>TS_ONEWIFI_5GHZ_WPA3RFCEnabled_WPA_RADIUS_On_Open_Psk_Enterprise</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>When RFC WPA3-Personal-Transition is enabled check WPA and Radius configuration on security mode transition(None->WPA3-Personal->WPA3-Enterprise) for 5GHZ</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_270</test_case_id>
    <test_objective>When WPA3_Personal_Transition RFC Feature is  enabled  and When security mode transitions from None to WPA3-Personal to WPA3-Enterprise (open-psk-enterprise), check if the WPA configuration (Encryption method, Preshared key) set for(None-->WPA3-Personal)gets reset for (WPA3-Personal-->WPA2-Enterprise). For 5GHZ</test_objective>
...
10. Revert to initial security mode. 11. unload the module/automation_approch>
    ...
    <test_script>TS_ONEWIFI_5GHZ_WPA3RFCEnabled_WPA_RADIUS_On_Open_Psk_Enterprise</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_WPA3RFCEnabled_WPA_RADIUS_On_Open_Psk_Enterprise')
sysobj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_WPA3RFCEnabled_WPA_RADIUS_On_Open_Psk_Enterprise')
        ...
        if expectedresult in actualresult:
            ...
            if pre_req_set == 1:
                ...
                if expectedresult in actualresult:
                    ...
                    #Check Security Mode  WPA and Radius Config parameters
                    step = step + 1
                    ...
                    print("TEST STEP %s: Check security mode WPA and Radius configuration" % step)
                    print("EXPECTED RESULT %s: WPA and Radius configuration values should be reset when security mode is None" % step)

                    if "FAILURE" not in actualresult_all and actual_values == expected_values:
                        tdkTestObj.setResultStatus("SUCCESS")
                        print("ACTUAL RESULT %s: SecurityMode is '%s',Encryption Mode is ' %s', PresharedKey is ' %s', KeyPasspharse is ' %s', RadiusIp is '%s', RadiusServerPort is '%s' , RadiusSecret is '%s' " % (step,actual_values[0],actual_values[1],actual_values[5],actual_values[6],actual_values[2],actual_values[3],actual_values[4]))
                        print("[TEST EXECUTION RESULT] : SUCCESS")

                        #Change Security Mode to WPA3-Personal, Encryption Mode to AES and wpa config parameters to new values
                        step = step + 1
                        ...
                        if expectedresult in actualresult:
                            ...
                            print(f"EXPECTED RESULT {step}: Security mode should be WPA3-Personal Encryption mode should be {ENCRYPTION_MODE} and KeyPassphrase should be 'Aa12345678'")
                            ...
                            if "FAILURE" not in actualresult_all and actual_values == expected_values:
                                ...
                                #Change Security Mode to WPA3-Enterprise , RadiusIP to 192.168.222.1 ,RadiusServerPort to 1812 and RadiusSecret to 'Aa12345678'
                                ...
                                if expectedresult in actualresult:
                                    tdkTestObj.setResultStatus("SUCCESS")
                                    print("ACTUAL RESULT %s: Security mode set to WPA3-Enterprise and Radius Configuration values are updated" % step)
                                    print("[TEST EXECUTION RESULT] : SUCCESS")
                                    ...
                                    if "FAILURE" not in actualresult_all and actual_values == expected_values:
                                        tdkTestObj.setResultStatus("SUCCESS")
                                        print("ACTUAL RESULT %s: Security mode is '%s', Encryption method is '%s', RadiusIp is '%s', RadiusServerPort is '%s', RadiusSecret is '%s' and PreSharedKey is ' %s'" % (step,actual_values[0],actual_values[1],actual_values[2],actual_values[3],actual_values[4],actual_values[5]))
                                        print("[TEST EXECUTION RESULT] : SUCCESS")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE")
                                        print("ACTUAL RESULT %s: Security mode is '%s', Encryption method is '%s', RadiusIp is '%s', RadiusServerPort is '%s', RadiusSecret is '%s' and PreSharedKey is ' %s'" % (step,actual_values[0],actual_values[1],actual_values[2],actual_values[3],actual_values[4],actual_values[5]))
                                    ...
                                    tdkTestObj.setResultStatus("FAILURE")
                                    print("ACTUAL RESULT %s: Failed to change SecurityMode WPA3-Enterprise and Radius configuration to new values" % step)
                                    print("[TEST EXECUTION RESULT] : FAILURE")
                            else:
                                tdkTestObj.setResultStatus("FAILURE")
                                print("ACTUAL RESULT %s: Security mode is '%s', Encryption method is '%s', PreSharedKey is ' %s', KeyPassphrase is ' %s'" % (step,actual_values[0],actual_values[1],actual_values[2],actual_values[3]))
                            ...
                            tdkTestObj.setResultStatus("FAILURE")
                            print("ACTUAL RESULT %s: Failed to change SecurityMode WPA3-Personal and WPA configuration to new values" % step)
                            print("[TEST EXECUTION RESULT] : FAILURE")
                    else:
                        tdkTestObj.setResultStatus("FAILURE")
                        print("ACTUAL RESULT %s: SecurityMode is '%s',Encryption Mode is ' %s', PresharedKey is ' %s', KeyPasspharse is ' %s', RadiusIp is '%s', RadiusServerPort is '%s' , RadiusSecret is '%s' " % (step,actual_values[0],actual_values[1],actual_values[5],actual_values[6],actual_values[2],actual_values[3],actual_values[4]))
