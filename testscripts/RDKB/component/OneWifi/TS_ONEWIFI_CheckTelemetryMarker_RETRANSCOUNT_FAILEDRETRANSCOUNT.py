...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port, 'TS_ONEWIFI_CheckTelemetryMarker_RETRANSCOUNT_FAILEDRETRANSCOUNT')
sysObj.configureTestCase(ip,port, 'TS_ONEWIFI_CheckTelemetryMarker_RETRANSCOUNT_FAILEDRETRANSCOUNT')
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in actualresult and tel_loginterval == "300":
                        ...
                        if expectedresult in actualresult and whl_found == 1:
                            ...
                            # Check for any WIFI_RETRANSCOUNT telemetry marker in wifihealth log file
                            step += 1
                            print(f"TEST STEP {step}: Check if WIFI_RETRANSCOUNT telemetry marker is present in wifihealth log file")
                            print(f"EXPECTED RESULT {step}: WIFI_RETRANSCOUNT telemetry marker should be present and value should be retrieved successfully")
                            ...
                            for i in range(1, 6):
                                if markerfound == 1:
                                    ...
                                    print(f"Checking WIFI_RETRANSCOUNT marker in wifihealth log file iteration {i}")
                                    query = "cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_RETRANSCOUNT_\" | grep -vi \"TOTAL_COUNT\" | tail -1"
                                    tdkTestObj = sysObj.createTestStep('ExecuteCmd')
                                    ...
                                    if len(details) == 0 or "WIFI_RETRANSCOUNT_" not in details:
                                        markerfound = 0
                                        sleep(60)
                                    else:
                                        marker_match = re.search(r"(WIFI_RETRANSCOUNT_[0-9]+):\s*([0-9]+)", details, re.IGNORECASE)

                                        if marker_match:
                                            retrans_marker_name = marker_match.group(1)
                                            tel_retranscount = marker_match.group(2).strip()
                                            markerfound = 1
                                        else:
                                            markerfound = 0
                                            sleep(60)
                            ...
                            if expectedresult in actualresult and markerfound == 1 and tel_retranscount != "":
                                tdkTestObj.setResultStatus("SUCCESS")
                                print(f"ACTUAL RESULT {step}: {retrans_marker_name} Marker is {tel_retranscount}")
                                print("[TEST EXECUTION RESULT] : SUCCESS")

                                step += 1
                                print(f"TEST STEP {step}: Check if RETRANSCOUNT telemetry marker value is greater than zero")
                                print(f"EXPECTED RESULT {step}: RETRANSCOUNT telemetry marker value should be greater than zero")

                                if int(tel_retranscount) > 0:
                                    tdkTestObj.setResultStatus("SUCCESS")
                                    print(f"ACTUAL RESULT {step}: RETRANSCOUNT telemetry marker value: {tel_retranscount}")
                                    print("[TEST EXECUTION RESULT] : SUCCESS")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE")
                                    print(f"ACTUAL RESULT {step}: RETRANSCOUNT telemetry marker value: {tel_retranscount}")
                                ...
                                # Check for any WIFI_FAILEDRETRANSCOUNT telemetry marker in wifihealth log file
                                step += 1
                                print(f"TEST STEP {step}: Check if WIFI_FAILEDRETRANSCOUNT telemetry marker is present in wifihealth log file")
                                print(f"EXPECTED RESULT {step}: WIFI_FAILEDRETRANSCOUNT telemetry marker should be present and value should be retrieved successfully")

                                markerfound = 0
                                tel_failedretranscount = ""
                                failedretrans_marker_name = ""

                                for i in range(1, 6):
                                    if markerfound == 1:
                                        break
                                    else:
                                        print(f"Checking WIFI_FAILEDRETRANSCOUNT marker in wifihealth log file iteration {i}")
                                        query = "cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_FAILEDRETRANSCOUNT_\" | grep -vi \"TOTAL_COUNT\" | tail -1"
                                        tdkTestObj = sysObj.createTestStep('ExecuteCmd')
                                        tdkTestObj.addParameter("command", query)
                                        tdkTestObj.executeTestCase(expectedresult)
                                        actualresult = tdkTestObj.getResult()
                                        ...
                                        if len(details) == 0 or "WIFI_FAILEDRETRANSCOUNT_" not in details:
                                            markerfound = 0
                                            sleep(60)
                                        else:
                                            marker_match = re.search(r"(WIFI_FAILEDRETRANSCOUNT_[0-9]+):\s*([0-9]+)", details, re.IGNORECASE)

                                            if marker_match:
                                                failedretrans_marker_name = marker_match.group(1)
                                                tel_failedretranscount = marker_match.group(2).strip()
                                                markerfound = 1
                                            else:
                                                markerfound = 0
                                                sleep(60)
                                ...
                                if expectedresult in actualresult and markerfound == 1 and tel_failedretranscount != "":
                                    tdkTestObj.setResultStatus("SUCCESS")
                                    print(f"ACTUAL RESULT {step}: {failedretrans_marker_name} Marker is {tel_failedretranscount}")
                                    print("[TEST EXECUTION RESULT] : SUCCESS")

                                    step += 1
                                    print(f"TEST STEP {step}: Check if FAILEDRETRANSCOUNT telemetry marker value is greater than zero")
                                    print(f"EXPECTED RESULT {step}: FAILEDRETRANSCOUNT telemetry marker value should be greater than zero")

                                    if int(tel_failedretranscount) > 0:
                                        tdkTestObj.setResultStatus("SUCCESS")
                                        print(f"ACTUAL RESULT {step}: FAILEDRETRANSCOUNT telemetry marker value: {tel_failedretranscount}")
                                        print("[TEST EXECUTION RESULT] : SUCCESS")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE")
                                        print(f"ACTUAL RESULT {step}: FAILEDRETRANSCOUNT telemetry marker value: {tel_failedretranscount}")
                                    ...
                                    tdkTestObj.setResultStatus("FAILURE")
                                    print(f"ACTUAL RESULT {step}: WIFI_FAILEDRETRANSCOUNT telemetry marker not found in wifihealth log file")
                                    print("[TEST EXECUTION RESULT] : FAILURE")
                            else:
                                tdkTestObj.setResultStatus("FAILURE")
                                print(f"ACTUAL RESULT {step}: WIFI_RETRANSCOUNT telemetry marker not found in wifihealth log file")
                                print("[TEST EXECUTION RESULT] : FAILURE")
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
