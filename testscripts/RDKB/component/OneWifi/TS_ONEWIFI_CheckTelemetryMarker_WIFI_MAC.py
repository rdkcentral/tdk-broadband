...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port, 'TS_ONEWIFI_CheckTelemetryMarker_WIFI_MAC')
sysObj.configureTestCase(ip,port, 'TS_ONEWIFI_CheckTelemetryMarker_WIFI_MAC')
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in sysutilloadmodulestatus.upper():
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in actualresult and tel_loginterval == "300":
                        ...
                        if expectedresult in actualresult and whl_found == 1:
                            ...
                            # Check for any WIFI_MAC telemetry marker in wifihealth log file
                            step += 1
                            print(f"TEST STEP {step}: Check if WIFI_MAC telemetry marker is present in wifihealth log file")
                            print(f"EXPECTED RESULT {step}: WIFI_MAC telemetry marker should be present and value should be retrieved successfully")
                            ...
                            for i in range(1, 6):
                                if markerfound == 1:
                                    ...
                                    print(f"Checking WIFI_MAC marker in wifihealth log file iteration {i}")
                                    ...
                                    if len(details) == 0 or "WIFI_MAC_" not in details:
                                        ...
                                        if marker_match:
                                            ...
                                            tel_wifimac = marker_match.group(2).strip()
                                            markerfound = 1
                                        else:
                                            markerfound = 0
                                            sleep(60)
                            ...
                            if expectedresult in actualresult and markerfound == 1 and tel_wifimac != "":
                                tdkTestObj.setResultStatus("SUCCESS")
                                print(f"ACTUAL RESULT {step}: {mac_marker_name} Marker is {tel_wifimac}")
                                print("[TEST EXECUTION RESULT] : SUCCESS")

                                step += 1
                                print(f"TEST STEP {step}: Validate WIFI_MAC telemetry marker value")
                                print(f"EXPECTED RESULT {step}: WIFI_MAC telemetry marker value should be non-empty and should be in valid MAC address format")

                                if re.match(r"^([A-Fa-f0-9]{2}:){5}[A-Fa-f0-9]{2}$", tel_wifimac):
                                    tdkTestObj.setResultStatus("SUCCESS")
                                    print(f"ACTUAL RESULT {step}: WIFI_MAC telemetry marker value is valid: {tel_wifimac}")
                                    print("[TEST EXECUTION RESULT] : SUCCESS")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE")
                                    print(f"ACTUAL RESULT {step}: WIFI_MAC telemetry marker value is invalid: {tel_wifimac}")
                                    print("[TEST EXECUTION RESULT] : FAILURE")
                            else:
                                tdkTestObj.setResultStatus("FAILURE")
                                print(f"ACTUAL RESULT {step}: WIFI_MAC telemetry marker not found in wifihealth log file")
                                print("[TEST EXECUTION RESULT] : FAILURE")
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
    ...
    print("Failed to load pam and sysutil module")
