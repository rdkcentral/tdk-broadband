...
<xml>
  ...
  <name>TS_ONEWIFI_5GHZ_WPA2_RADIUS_WPA_On_Open_Enterprise_Psk_SecurityModeTransition</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check whether RADIUS Configuration will be reset or not on security mode transition(open-enterprise-psk) for 5GHZ</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_275</test_case_id>
    <test_objective>To check whether WPA Configuration will be reset or not on security mode transition(open-enterprise-psk) for 5GHZ (AP1)</test_objective>
...
5. Change the security mode to WPA2-Enterprise ,Encryption method to AES and Radius serverIP, Serverport, RadiusSecret to new values.
6. Check if mode set is reflected in get and if the RADIUS configuration is set to the new values.
7. Now set to the security  mode WPA2-Personal encryption method to AES, Preshared key to new value.
...
10. Unload the module</automation_approch>
    <expected_output>When security mode changes for open to enterprise(None to WPA2-Enterprise) RADIUS config should set. When security mode changes from enterprise to psk (WPA2-Enterprise to WPA2-Personal ) RADIUS config should reset.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WifiAgent</test_stub_interface>
    <test_script>TS_ONEWIFI_5GHZ_WPA2_RADIUS_WPA_On_Open_Enterprise_Psk_SecurityModeTransition</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_WPA2_RADIUS_WPA_On_Open_Enterprise_Psk_SecurityModeTransition')
sysobj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_WPA2_RADIUS_WPA_On_Open_Enterprise_Psk_SecurityModeTransition')
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult:
                ...
                if "FAILURE" not in actualresult_all and expected_values == actual_values:
                    ...
                    #Change security mode to WPA2-Enterprise, encryption method to AES and Radius config parameters to new values
                    ...
                    print("TEST STEP 6: Set security mode to WPA2-Enterprise and RADIUS configuration to new values")
                    print(f"EXPECTED RESULT 6: Should set security mode to WPA2-Enterprise Encryption mode to {ENCRYPTION_MODE}, RADIUSserverIP to {RADIUS_IP}, RADIUSserverPort to {RADIUS_PORT} and RADIUSsecret to {RADIUS_SECRET}")

                    if expectedresult in actualresult:
                        tdkTestObj.setResultStatus("SUCCESS")
                        print("ACTUAL RESULT 6: Security mode set to WPA2-Enterprise and RADIUS configuration with new values")
                        print("[TEST EXECUTION RESULT] : SUCCESS")
                        ...
                        print("EXPECTED RESULT 7: Security mode should be WPA2-Enterprise, RADIUS config parameters with new values")
                        ...
                        if "FAILURE" not in actualresult_all and actual_values == expected_values:
                            ...
                            #Set security mode to WPA2-Personal and WPA config parameters to new values
                            ...
                            actualresult = tdkTestObj.getResult()
                            sleep(1)
                            print("TEST STEP 8: Set security mode to WPA2-Personal and WPA configuration to new values")
                            print(f"EXPECTED RESULT 8: Should set security mode to WPA2-Personal Encryption mode to {ENCRYPTION_MODE}  and PreSharedKey to'Aa12345678'")

                            if expectedresult in actualresult:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print("ACTUAL RESULT 8: Security mode set to WPA2-Personal and WPA configuration with new values")
                                print("[TEST EXECUTION RESULT] : SUCCESS")
                                ...
                                print("EXPECTED RESULT 9: Security mode should be WPA2-Personal, WPA config parameters with new values")
    ...
    if revertflag == 1:
        ...
        print("Reverting prerequites not required")
