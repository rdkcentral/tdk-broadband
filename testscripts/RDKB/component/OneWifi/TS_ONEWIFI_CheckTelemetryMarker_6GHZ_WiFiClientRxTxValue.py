...
<xml>
  ...
  <name>TS_ONEWIFI_CheckTelemetryMarker_6GHZ_WiFiClientRxTxValue</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check for the telemetry marker WIFI_RXCLIENTS_17 and WIFI_TXCLIENTS_17 are populating with non-empty values in log file /rdklogs/logs/wifihealth.txt</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_299</test_case_id>
    <test_objective>Check for the telemetry marker WIFI_RXCLIENTS_17 and WIFI_TXCLIENTS_17 in log file /rdklogs/logs/wifihealth.txt and check if it is populating with non empty values</test_objective>
...
5.After 5mins grep "WIFI_RXCLIENT_17" and "WIFI_TXCLIENT_17" in wifihealth.txt file and it should be non empty values
6.Unload module</automation_approch>
    <expected_output>Telemetry marker "WIFI_RX_CLIENT_17"and "WIFI_TXCLIENT_17" should be populated in wifihealth.txt and it should be populated with non empty values
</expected_output>
    <priority></priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_ONEWIFI_CheckTelemetryMarker_6GHZ_WiFiClientRxTxValue</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_CheckTelemetryMarker_6GHZ_WiFiClientRxTxValue')
sysObj.configureTestCase(ip,port,'TS_ONEWIFI_CheckTelemetryMarker_6GHZ_WiFiClientRxTxValue')
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in actualresult and tel_loginterval == "300":
                        ...
                        if expectedresult in actualresult and whl_found == 1:
                            ...
                            #Check for WIFI_TXCLIENTS_17 Marker in wifihealth log file
                            step += 1
                            print(f"TEST STEP {step}: Check if WIFI_TXCLIENTS_17 Marker presence in wifihealth log file")
                            print(f"EXPECTED RESULT {step}: WIFI_TXCLIENTS_17 Marker should be present wifihealth log file")
                            markerfound = 0
                            for i in range(1, 6):
                                if markerfound == 1:
                                    break
                                else:
                                    print(f"Checking WIFI_TXCLIENTS_17 in wifihealth log file iteration {i}")
                                    query = "cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_TXCLIENTS_17:\""
                                    tdkTestObj = sysObj.createTestStep('ExecuteCmd')
                                    tdkTestObj.addParameter("command", query)
                                    tdkTestObj.executeTestCase(expectedresult)
                                    actualresult = tdkTestObj.getResult()
                                    details = tdkTestObj.getResultDetails()
                                    if (len(details) == 0) or details.endswith(":") or "WIFI_TXCLIENTS_17" not in details:
                                        markerfound = 0
                                        sleep(60)
                                    else:
                                        tel_txclients = details.split("WIFI_TXCLIENTS_17:")[1].split(',')[0].replace("\\n", "").strip()
                                        markerfound = 1

                            if expectedresult in actualresult and markerfound == 1 and tel_txclients != "":
                                tdkTestObj.setResultStatus("SUCCESS")
                                print(f"ACTUAL RESULT {step}: WIFI_TXCLIENTS_17 Marker is {tel_txclients}")
                                print("[TEST EXECUTION RESULT] : SUCCESS")

                                #chek for WIFI_RXCLIENTS_17 Marker is present in wifihealth logfile
                                step += 1
                                print(f"TEST STEP {step}: Check if WIFI_RXCLIENTS_17 Marker is present in wifihealth log")
                                print(f"EXPECTED RESULT {step}: WIFI_RXCLIENTS_17 Marker should be present wifihealth log")
                                markerfound = 0
                                for i in range(1, 6):
                                    if markerfound == 1:
                                        break
                                    else:
                                        print(f"Checking WIFI_RXCLIENTS_17 in wifihealth log file iteration {i}")
                                        query = "cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_RXCLIENTS_17:\""
                                        tdkTestObj = sysObj.createTestStep('ExecuteCmd')
                                        tdkTestObj.addParameter("command", query)
                                        tdkTestObj.executeTestCase(expectedresult)
                                        actualresult = tdkTestObj.getResult()
                                        details = tdkTestObj.getResultDetails()
                                        if (len(details) == 0) or details.endswith(":") or "WIFI_RXCLIENTS_17" not in details:
                                            markerfound = 0
                                            sleep(60)
                                        else:
                                            tel_rxclients = details.split("WIFI_RXCLIENTS_17:")[1].split(',')[0].replace("\\n", "").strip()
                                            markerfound = 1
                                if expectedresult in actualresult and markerfound == 1 and tel_rxclients != "":
                                    tdkTestObj.setResultStatus("SUCCESS")
                                    print(f"ACTUAL RESULT {step}: WIFI_RXCLIENTS_17 Marker is {tel_rxclients}")
                                    print("[TEST EXECUTION RESULT] : SUCCESS")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE")
                                    print(f"ACTUAL RESULT {step}: WIFI_RXCLIENTS_17 Marker not found in wifihealth log file or populated with empty value")
                                    print("[TEST EXECUTION RESULT] : FAILURE")
                            else:
                                tdkTestObj.setResultStatus("FAILURE")
                                print(f"ACTUAL RESULT {step}: WIFI_TXCLIENTS_17 Marker not found in wifihealth log file or populated with empty value")
                                print("[TEST EXECUTION RESULT] : FAILURE")
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
