...
<xml>
  ...
  <name>TS_ONEWIFI_CheckTelemetryMarkerWIFI_ACS_2</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check WIFI_ACS_2 is populating correctly in wifihealth.txt</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_250</test_case_id>
    <test_objective>To check WIFI_ACS_2 is populated correctly in wifihealth.txt</test_objective>
    <test_type></test_type>
...
2.Get the AutoChannelEnable by using Device.WiFi.Radio.2.AutoChannelEnable and store it
...
6.Check if WIFI_ACS_2 is present in wifihealth.txt. If not found, check every 60s for 15 minutes to see if the log is getting populated.
7.Retrive the value of WIFI_ACS_2.
8.Compare the initially retrived AutoChannelEnable with WIFI_ACS_2.
9.Unload the module.
</automation_approch>
    <expected_output>WIFI_ACS_2 should populate in wifihealth.txt and should match with AutoChannelEnable</expected_output>
    <priority></priority>
    <test_stub_interface>WifiAgent</test_stub_interface>
    <test_script>TS_ONEWIFI_CheckTelemetryMarkerWIFI_ACS_2</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
wifiobj.configureTestCase(ip,port,'TS_ONEWIFI_CheckTelemetryMarkerWIFI_ACS_2')
pamobj.configureTestCase(ip,port,'TS_ONEWIFI_CheckTelemetryMarkerWIFI_ACS_2')
sysObj.configureTestCase(ip,port,'TS_ONEWIFI_CheckTelemetryMarkerWIFI_ACS_2')
...
if "SUCCESS" in wifiloadmodulestatus.upper() and "SUCCESS" in pamloadmodulestatus.upper() and "SUCCESS" in sysutilloadmodulestatus.upper():
    ...
    paramName = "Device.WiFi.Radio.2.AutoChannelEnable"
                    ...
                    if expectedresult in actualresult:
                        ...
                        if expectedresult in actualresult:
                            ...
                            if details == "File exist":
                                ...
                                #Check for the maraker WIFI_ACS_2
                                step = step + 1
                                tdkTestObj = sysObj.createTestStep('ExecuteCmd')
                                cmd = "grep -ire \"WIFI_ACS_2\" /rdklogs/logs/wifihealth.txt"
                                tdkTestObj.addParameter("command",cmd)
                                expectedresult="SUCCESS"

                                print("\nTEST STEP %s: Check for the presence of the marker WIFI_ACS_2 in wifihealth.txt" % step)
                                print("EXPECTED RESULT %s: WIFI_ACS_2 marker should be present in wifihealth.txt" % step)
                                ...
                                for iteration in range(1,6):
                                    ...
                                    if expectedresult in actualresult and "WIFI_ACS_2" in details:
                                        markerfound = 1
                                        break
                                    else:
                                        sleep(60)
                                ...
                                if markerfound == 1:
                                    tdkTestObj.setResultStatus("SUCCESS")
                                    print("ACTUAL RESULT %s: WIFI_ACS_2 marker is found in wifihealth.txt : %s" %(step,details))
                                    print("[TEST EXECUTION RESULT] : SUCCESS")

                                    # Retrieve the value of WIFI_ACS_2
                                    step += 1
                                    tdkTestObj = sysObj.createTestStep('ExecuteCmd')
                                    cmd = "grep -inr 'WIFI_ACS_2' /rdklogs/logs/wifihealth.txt | tail -1 | awk -F 'WIFI_ACS_2:' '{print $2}' | awk '{print $1}'"
                                    tdkTestObj.addParameter("command", cmd)

                                    print("\nTEST STEP %s: Retrieve the value of WIFI_ACS_2" % step)
                                    print("EXPECTED RESULT %s: The value of WIFI_ACS_2 should be retrieved" % step)
                                    ...
                                    if expectedresult in actualresult and details:
                                        tdkTestObj.setResultStatus("SUCCESS")
                                        print("ACTUAL RESULT %s: Retrieved the value of WIFI_ACS_2: %s" % (step, details))
                                        print("[TEST EXECUTION RESULT] : SUCCESS")

                                        #compare WIFI_ACS_2 with AutochannelEnable
                                        step = step + 1
                                        print("\nTEST STEP %s: Compare WIFI_ACS_2 with initially retrived AutochannelEnable" % step)
                                        print("EXPECTED RESULT %s: WIFI_ACS_2 and initially retrived AutochannelEnable should has same value" % step)

                                        if details == initial_AutoChannel:
                                            tdkTestObj.setResultStatus("SUCCESS")
                                            print("The value of AutoChannelEnable is %s" % initial_AutoChannel)
                                            print("The value of WIFI_ACS_2 in wifihealth.txt is %s" % details)
                                            print("ACTUAL RESULT %s: WIFI_ACS_2 and AutoChannelEnable is same" % step)
                                            print("[TEST EXECUTION RESULT] : SUCCESS")
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE")
                                            print("The value of initial AutoChannelEnable is %s" % initial_AutoChannel)
                                            print("The value of WIFI_ACS_2 in wifihealth.txt is %s" % details)
                                            print("ACTUAL RESULT %s: WIFI_ACS_2 and AutoChannelEnable is not same" % step)
                                            print("[TEST EXECUTION RESULT] : FAILURE")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE")
                                        print("ACTUAL RESULT %s: Failed to retrieve the value of WIFI_ACS_2" % step)
                                        print("[TEST EXECUTION RESULT] : FAILURE")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE")
                                    print("ACTUAL RESULT %d: WIFI_ACS_2 marker is not found Details : %s" %(step,details))
                                    print("[TEST EXECUTION RESULT] : FAILURE")
                            else:
                                tdkTestObj.setResultStatus("FAILURE")
