...
<xml>
  ...
  <name>TS_ONEWIFI_5GHZ_WPAmode_RADIUS_WPA_On_Open_Enterprise_Psk_SecurityModeTransition</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check RADIUS and WPA configurations on security mode transition(None->WPA-Enterprise->WPA-Personal) for 5ghz</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_272</test_case_id>
    <test_objective>To check whether RADIUS and WPA configuration will be reset or not on security mode transition(None->WPA-Enterprise->WPA-Personal) for 5ghz</test_objective>
...
5. Change the security mode to WPA-Enterprise ,Encryption method to AES and Radius serverIP, Serverport, RadiusSecret to new values.
6. Check if mode set is reflected in get and if the RADIUS configuration is set to the new values.
7. Now set to the security mode to  WPA-Personal encryption method to AES, Preshared key to new value.
8. Check if security mode set is reflected in get and if the RADIUS configuration is reset
...
10. unload the module</automation_approch>
    ...
    <test_script>TS_ONEWIFI_5GHZ_WPAmode_RADIUS_WPA_On_Open_Enterprise_Psk_SecurityModeTransition</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_WPAmode_RADIUS_WPA_On_Open_Enterprise_Psk_SecurityModeTransition')
sysobj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_WPAmode_RADIUS_WPA_On_Open_Enterprise_Psk_SecurityModeTransition')
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult:
                ...
                if "FAILURE" not in actualresult_all and expected_values == actual_values:
                    ...
                    #Change security mode to WPA-Enterprise, encryption method to AES and radius config parameters to new values
                    ...
                    print("\nTEST STEP 6: Set security mode to WPA-Enterprise and RADIUS configuration to new values")
                    print(f"EXPECTED RESULT 6: Should set security mode to WPA-Enterprise Encryption mode to {ENCRYPTION_MODE}, RADIUSserverIP to {RADIUS_IP}, RADIUSserverPort to {RADIUS_PORT} and RADIUSsecret to {RADIUS_SECRET}")

                    if expectedresult in actualresult:
                        tdkTestObj.setResultStatus("SUCCESS")
                        print("ACTUAL RESULT 6: Security mode set to WPA-Enterprise and RADIUS configuration with new values")
                        print("[TEST EXECUTION RESULT] : SUCCESS")
                        ...
                        paramName = "Device.WiFi.AccessPoint.2.Security.X_CISCO_COM_EncryptionMethod"
                        tdkTestObj,actualresult,encryptionMethod = wifi_GetParam(obj,paramName)
                        tdkTestObj,actualresult,radiusIP,radiusPort,radiusSecret = get_radiusconfig_params(obj,ap_index)

                        print("\nTEST STEP 7: Check security mode and RADIUS configuration Values")
                        print("EXPECTED RESULT 7: Security mode should be WPA-Enterprise, RADIUS config parameters with new values")

                        if expectedresult in actualresult and encryptionMethod == ENCRYPTION_MODE and radiusIP == RADIUS_IP and radiusPort == RADIUS_PORT and radiusSecret =="":
                            tdkTestObj.setResultStatus("SUCCESS")
                            ...
                            #Set security mode to WPA-Personal and WPA config parameters to new values
                            ...
                            print("TEST STEP 8: Set security mode to WPA-Personal and WPA configuration to new values")
                            print(f"EXPECTED RESULT 8: Should set security mode to WPA-Personal Encryption mode to {ENCRYPTION_MODE} and PreSharedKey to {PRESHAREDKEY}")

                            if expectedresult in actualresult:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print("ACTUAL RESULT 8: Security mode set to WPA-Personal and WPA configuration with new values")
                                print("[TEST EXECUTION RESULT] : SUCCESS")
                                ...
                                print("\nTEST STEP 9: Check SecurityMode and WPA Config Paremeters")
                                print("EXPECTED RESULT 9: Security mode should be WPA-Personal, WPA config parameters with new values")
                                ...
                                tdkTestObj.setResultStatus("FAILURE")
                                print("ACTUAL RESULT 8: Failed to change security mode set to WPA-Personal and WPA configuration to new values")
                                print("[TEST EXECUTION RESULT] : FAILURE")
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
                            print("ACTUAL RESULT 7: Security mode is '%s',  Encryption method is ' %s', RADIUS IP is ' %s', RADIUS ServerPort is ' %s', RADIUSSecret is ' %s'" % (securityMode,encryptionMethod,radiusIP,radiusPort,radiusSecret))
                        ...
                        tdkTestObj.setResultStatus("FAILURE")
                        print("ACTUAL RESULT 6: Failed to change security mode set to WPA-Enterprise and RADIUS configuration to new values")
                        print("[TEST EXECUTION RESULT] : FAILURE")
                else:
                    tdkTestObj.setResultStatus("FAILURE")
                    print("ACTUAL RESULT 5: SecurityMode is '%s',Encryption Mode is ' %s', PresharedKey is ' %s', KeyPasspharse is ' %s', RADIUS IP is ' %s', RADIUS ServerPort is ' %s', RADIUS Secret is ' %s'" % (actual_values[0],actual_values[1],actual_values[5],actual_values[6],actual_values[2],actual_values[3],actual_values[4]))
