...
<xml>
  ...
  <name>TS_ONEWIFI_5GHZ_WPA-WPA2_RADIUS_WPA_On_Open_Psk_Enterprise_SecurityModeTransition</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check RADIUS and WPA configurations on security mode transition(None->WPA-WPA2-Personal->WPA-WPA2-Enterprise) for 5ghz</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_266</test_case_id>
    <test_objective>To check whether RADIUS and WPA configuration will be reset or not on security mode transition(None->WPA-WPA2-Personal->WPA-WPA2-Enterprise) for 5ghz</test_objective>
    ...
    5. Change the security mode to WPA-WPA2-Personal ,Encryption method to AES and Preshared key to a new value.
    6. Check if mode set is reflected in get and if the WPA configuration is set to the new values.
    7. Now set to the enterprise mode WPA-WPA2-Enterprise encryption method to AES, radius parameters: radius server, radius secret, radius port to valid values.
    ...
    <expected_output>When security mode changes form open to psk (None to WPA-WPA2-Personal) WPA config should set and When security mode changes from psk to enterprise(WPA-WPA2-Personal to WPA-WPA2-Enterprise) WPA config should reset</expected_output>
    <priority>High</priority>
    <test_stub_interface>WifiAgent</test_stub_interface>
    <test_script>TS_ONEWIFI_5GHZ_WPA-WPA2_RADIUS_WPA_On_Open_Psk_Enterprise_SecurityModeTransition</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_WPA-WPA2_RADIUS_WPA_On_Open_Psk_Enterprise_SecurityModeTransition')
sysobj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_WPA-WPA2_RADIUS_WPA_On_Open_Psk_Enterprise_SecurityModeTransition')
    ...
    if "FAILURE" not in actualresult_all:
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult:
                ...
                print("TEST STEP 5: Check security mode RADIUS and WPA configuration")
                ...
                if "FAILURE" not in actualresult_all:
                    ...
                    # Change security mode to WPA-WPA2-Personal, encryption method to AES and PresharedKey to Aa12345678
                    tdkTestObj = obj.createTestStep("WIFIAgent_SetMultiple")
                    paramList = ("Device.WiFi.AccessPoint.2.Security.ModeEnabled|WPA-WPA2-Personal|string|"
                                "Device.WiFi.AccessPoint.2.Security.X_CISCO_COM_EncryptionMethod|%s|string|"
                                "Device.WiFi.AccessPoint.2.Security.PreSharedKey|Aa12345678|string" % ENCRYPTION_MODE)
                    tdkTestObj.addParameter("paramList", paramList)
                    tdkTestObj.executeTestCase(expectedresult)
                    actualresult = tdkTestObj.getResult()
                    sleep(2)

                    print("\nTEST STEP 6: Change security mode to WPA-WPA2-Personal, Encryption mode to AES and PresharedKey to 'Aa12345678' ")
                    print("EXPECTED RESULT 6: Should set security mode to WPA-WPA2-Personal and WPA config parameters to new values")

                    if expectedresult in actualresult:
                        tdkTestObj.setResultStatus("SUCCESS")
                        print("ACTUAL RESULT 6: Security mode set to WPA-WPA2-Personal, WPA config parameters are updated")
                        print("[TEST EXECUTION RESULT] : SUCCESS")
                        ...
                        print("EXPECTED RESULT 7: Security mode should be WPA-WPA2-Personal Encryption mode should be 'AES' and KeyPassphrase should be 'Aa12345678'")
                        ...
                        if "FAILURE" not in actualresult_all and actual_values == expected_values:
                            ...
                            #Set security mode to WPA-WPA2-Enterprise and RADIUS config parameters to new values

                            tdkTestObj = obj.createTestStep("WIFIAgent_SetMultiple")
                            paramList = ("Device.WiFi.AccessPoint.2.Security.ModeEnabled|WPA-WPA2-Enterprise|string|"
                                "Device.WiFi.AccessPoint.2.Security.X_CISCO_COM_EncryptionMethod|%s|string|"
                            ...
                            print("\nTEST STEP 8: Set security mode to WPA-WPA2-Enterprise and RADIUS configuration to new values")
                            print(f"EXPECTED RESULT 8: Should set security mode to WPA-WPA2-Enterprise Encryption mode to {ENCRYPTION_MODE}, RADIUSserverIP to {RADIUS_IP}, RADIUSserverPort to {RADIUS_PORT} and RADIUSsecret to {RADIUS_SECRET}")

                            if expectedresult in actualresult:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print("ACTUAL RESULT 8: Security mode set to WPA-WPA2-Enterprise and RADIUS configuration with new values")
                                print("[TEST EXECUTION RESULT] : SUCCESS")
                                ...
                                print("EXPECTED RESULT 9: Security mode should be WPA-WPA2-Enterprise, RADIUS config parameters with new values and PreSharedKey should be empty")
