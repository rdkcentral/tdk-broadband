...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>5</version>
  <name>TS_ONEWIFI_5GHzSetSupportedSecurityModes_WPA3PersonalTransitionEnabled</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_177</test_case_id>
...
5. Get the supported modes using Device.WiFi.AccessPoint.2.Security.ModesSupported.
6. Check if the SET operation of Device.WiFi.AccessPoint.2.Security.ModeEnabled is success for all supported modes that can be set for AP 2. Ensure security configurations are set if required for mode transition.
7. Revert the WPA3 RFC Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.WPA3_Personal_Transition.Enable status if required.
8. Revert the security mode enabled and the security configuration values to initial values if required.
9. Unload the module.</automation_approch>
    ...
    <test_script>TS_ONEWIFI_5GHzSetSupportedSecurityModes_WPA3PersonalTransitionEnabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_5GHzSetSupportedSecurityModes_WPA3PersonalTransitionEnabled');
