...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port, 'TS_ONEWIFI_CheckTelemetryMarker_BytesSentAndReceived')
sysObj.configureTestCase(ip,port, 'TS_ONEWIFI_CheckTelemetryMarker_BytesSentAndReceived')
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in actualresult:
                    ...
                    if details == "File exist":
                        ...
                        markerfound = 0
                        tel_bytessent = ""
                        bytes_sent_marker_name = ""

                        for i in range(1, 6):
                            if markerfound == 1:
                                break
                            else:
                                # Query for any WIFI_BYTESSENTCLIENTS telemetry marker
                                query = "cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_BYTESSENTCLIENTS_\" | grep -vi \"TOTAL_COUNT\" | tail -1"
                                print("query:%s" % query)
                                ...
                                if len(details) == 0 or "WIFI_BYTESSENTCLIENTS_" not in details:
                                    markerfound = 0
                                    sleep(60)
                                else:
                                    marker_match = re.search(r"(WIFI_BYTESSENTCLIENTS_[0-9]+):\s*([0-9]+)", details, re.IGNORECASE)

                                    if marker_match:
                                        bytes_sent_marker_name = marker_match.group(1)
                                        tel_bytessent = marker_match.group(2).strip()
                                        markerfound = 1
                                    else:
                                        markerfound = 0
                                        sleep(60)
                        ...
                        if expectedresult in actualresult and markerfound == 1 and tel_bytessent != "":
                            tdkTestObj.setResultStatus("SUCCESS")
                            print("TEST STEP 5: WIFI_BYTESSENTCLIENTS telemetry marker should be present")
                            print("EXPECTED RESULT 5: WIFI_BYTESSENTCLIENTS telemetry marker should be present and value should be retrieved")
                            print("ACTUAL RESULT 5: %s Marker is %s" % (bytes_sent_marker_name, tel_bytessent))
                            print("[TEST EXECUTION RESULT] : SUCCESS")

                            if int(tel_bytessent) > 0:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print("TEST STEP 6: Check if BYTESSENT telemetry marker value is greater than zero")
                                print("EXPECTED RESULT 6: BYTESSENT telemetry marker value should be greater than zero")
                                print("ACTUAL RESULT 6: BYTESSENT telemetry marker value:", tel_bytessent)
                                print("[TEST EXECUTION RESULT] : SUCCESS")
                            else:
                                tdkTestObj.setResultStatus("FAILURE")
                                print("TEST STEP 6: Check if BYTESSENT telemetry marker value is greater than zero")
                            ...
                            markerfound = 0
                            tel_bytesreceived = ""
                            bytes_received_marker_name = ""

                            for i in range(1, 6):
                                if markerfound == 1:
                                    break
                                else:
                                    # Query for any WIFI_BYTESRECEIVEDCLIENTS telemetry marker
                                    query = "cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_BYTESRECEIVEDCLIENTS_\" | grep -vi \"TOTAL_COUNT\" | tail -1"
                                    print("query:%s" % query)
                                    ...
                                    if len(details) == 0 or "WIFI_BYTESRECEIVEDCLIENTS_" not in details:
                                        markerfound = 0
                                        sleep(60)
                                    else:
                                        marker_match = re.search(r"(WIFI_BYTESRECEIVEDCLIENTS_[0-9]+):\s*([0-9]+)", details, re.IGNORECASE)

                                        if marker_match:
                                            bytes_received_marker_name = marker_match.group(1)
                                            tel_bytesreceived = marker_match.group(2).strip()
                                            markerfound = 1
                                        else:
                                            markerfound = 0
                                            sleep(60)
                            ...
                            if expectedresult in actualresult and markerfound == 1 and tel_bytesreceived != "":
                                tdkTestObj.setResultStatus("SUCCESS")
                                print("TEST STEP 7: WIFI_BYTESRECEIVEDCLIENTS telemetry marker should be present")
                                print("EXPECTED RESULT 7: WIFI_BYTESRECEIVEDCLIENTS telemetry marker should be present and value should be retrieved")
                                print("ACTUAL RESULT 7: %s Marker is %s" % (bytes_received_marker_name, tel_bytesreceived))
                                print("[TEST EXECUTION RESULT] : SUCCESS")

                                if int(tel_bytesreceived) > 0:
                                    tdkTestObj.setResultStatus("SUCCESS")
                                    print("TEST STEP 8: Check if BYTESRECEIVED telemetry marker value is greater than zero")
                                    print("EXPECTED RESULT 8: BYTESRECEIVED telemetry marker value should be greater than zero")
                                    print("ACTUAL RESULT 8: BYTESRECEIVED telemetry marker value:", tel_bytesreceived)
                                    print("[TEST EXECUTION RESULT] : SUCCESS")
                                else:
                                    tdkTestObj.setResultStatus("FAILURE")
                                    print("TEST STEP 8: Check if BYTESRECEIVED telemetry marker value is greater than zero")
                                ...
                                tdkTestObj.setResultStatus("FAILURE")
                                print("TEST STEP 7: WIFI_BYTESRECEIVEDCLIENTS telemetry marker should be present")
                                print("EXPECTED RESULT 7: WIFI_BYTESRECEIVEDCLIENTS telemetry marker should be present")
                                print("ACTUAL RESULT 7: WIFI_BYTESRECEIVEDCLIENTS telemetry marker not present")
                                print("[TEST EXECUTION RESULT] : FAILURE")
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
                            print("TEST STEP 5: WIFI_BYTESSENTCLIENTS telemetry marker should be present")
                            print("EXPECTED RESULT 5: WIFI_BYTESSENTCLIENTS telemetry marker should be present")
                            print("ACTUAL RESULT 5: WIFI_BYTESSENTCLIENTS telemetry marker not present")
                            print("[TEST EXECUTION RESULT] : FAILURE")
                    else:
                        tdkTestObj.setResultStatus("FAILURE")
                        print("TEST STEP 4: Check for wifihealth log file presence")
