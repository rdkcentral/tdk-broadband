...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        if "FAILURE" not in actualresult_all and dfs_rfc == "true" and dfs_enable == "true":
            ...
            # Step 3: Get current Radio 3 channel
            step += 1
            tdkTestObj, actualresult_ch, initial_radio3_channel = wifi_GetParam(obj, "Device.WiFi.Radio.3.Channel")
            print(f"\nTEST STEP {step}: Get current Radio 3 (6 GHz) channel")
            print(f"EXPECTED RESULT {step}: Should get current channel value for Radio 3")
            if expectedresult in actualresult_ch:
                tdkTestObj.setResultStatus("SUCCESS")
                print(f"ACTUAL RESULT {step}: Radio 3 current channel is {initial_radio3_channel}")
                print("TEST EXECUTION RESULT : SUCCESS")

                # Step 4: Get PossibleChannels for Radio 3
                step += 1
                tdkTestObj, actualresult_pc, radio3_possible_channels = wifi_GetParam(obj, "Device.WiFi.Radio.3.PossibleChannels")
                print(f"\nTEST STEP {step}: Get PossibleChannels for Radio 3 (6 GHz)")
                print(f"EXPECTED RESULT {step}: Should get possible channels list for Radio 3")
                if expectedresult in actualresult_pc:
                    ...
                    # Step 5: Disable Auto Channel Selection on Radio 3
                    step += 1
                    tdkTestObj, actualresult = wifi_SetParam(obj, "Device.WiFi.Radio.3.AutoChannelEnable", "false", "boolean")
                    sleep(2)
                    print(f"\nTEST STEP {step}: Disable Auto Channel Selection on Radio 3")
                    ...
                    if expectedresult in actualresult:
                        ...
                        if expectedresult in actualresult:
                            ...
                            # Step 7: Apply radio settings for Radio 3
                            step += 1
                            tdkTestObj, actualresult = wifi_SetParam(obj, "Device.WiFi.ApplyRadioSettings", "true", "boolean")
                            sleep(5)
                            print(f"\nTEST STEP {step}: Apply radio settings for Radio 3")
                            ...
                            # Step 8: Verify Radio 3 channel is updated
                            step += 1
                            tdkTestObj, actualresult, radio3_channel_after = wifi_GetParam(obj, "Device.WiFi.Radio.3.Channel")
                            print(f"\nTEST STEP {step}: Verify Radio 3 channel is updated to {target_channel}")
                    ...
                    tdkTestObj.setResultStatus("FAILURE")
                    print(f"ACTUAL RESULT {step}: GET PossibleChannels FAILURE for Radio 3")
                    print("TEST EXECUTION RESULT : FAILURE")
            else:
                tdkTestObj.setResultStatus("FAILURE")
                print(f"ACTUAL RESULT {step}: GET Radio 3 channel FAILURE")
                print("TEST EXECUTION RESULT : FAILURE")
        else:
            tdkTestObj.setResultStatus("FAILURE")
