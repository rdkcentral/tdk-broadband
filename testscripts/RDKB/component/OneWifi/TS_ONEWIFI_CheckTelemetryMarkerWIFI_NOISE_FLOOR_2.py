...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_ONEWIFI_CheckTelemetryMarkerWIFI_NOISE_FLOOR_2</name>
  <primitive_test_id/>
  <primitive_test_name>onewifi_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Check if WIFI_NOISE_FLOOR_2 is populated in wifihealth.txt</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_248</test_case_id>
    <test_objective>Check if WIFI_NOISE_FLOOR_2 is populated in wifihealth.txt</test_objective>
...
5.Check if the WIFI_NOISE_FLOOR_2 is present in wifihealth.txt. If not found, check every 60s for 5 minutes to see if the log is getting populated.
6.Retrive the value of WIFI_NOISE_FLOOR_2
...
7. Unload the modules.</automation_approch>
    <expected_output>The WIFI_NOISE_FLOOR_2 marker should present in wifihealth.txt</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_ONEWIFI_CheckTelemetryMarkerWIFI_NOISE_FLOOR_2</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_CheckTelemetryMarkerWIFI_NOISE_FLOOR_2');
sysObj.configureTestCase(ip,port,'TS_ONEWIFI_CheckTelemetryMarkerWIFI_NOISE_FLOOR_2');
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in actualresult:
                    ...
                    if details == "File exist":
                        ...
                        #Check for the maraker WIFI_NOISE_FLOOR_2
                        step = 5;
                        tdkTestObj = sysObj.createTestStep('ExecuteCmd');
                        cmd = "grep -ire \"WIFI_NOISE_FLOOR_2\" /rdklogs/logs/wifihealth.txt";
                        tdkTestObj.addParameter("command",cmd);
                        expectedresult="SUCCESS";

                        print("\nTEST STEP %d: Check for the presence of the marker WIFI_NOISE_FLOOR_2" %step);
                        print("EXPECTED RESULT %d: WIFI_NOISE_FLOOR_2 marker should be present" %step);
                        ...
                        for iteration in range(1,6):
                            ...
                            if expectedresult in actualresult and "WIFI_NOISE_FLOOR_2" in details:
                                markerfound = 1;
                                break;
                            else:
                                sleep(60);
                        ...
                        if markerfound == 1:
                            tdkTestObj.setResultStatus("SUCCESS");
                            print("ACTUAL RESULT %d: WIFI_NOISE_FLOOR_2 marker is found; Details : %s" %(step,details));
                            #Get the result of execution
                            print("[TEST EXECUTION RESULT] : SUCCESS");

                            # Retrieve the value of WIFI_NOISE_FLOOR_2
                            step += 1;
                            tdkTestObj = sysObj.createTestStep('ExecuteCmd')
                            cmd = "grep -ire 'WIFI_NOISE_FLOOR_2' /rdklogs/logs/wifihealth.txt | awk -F 'WIFI_NOISE_FLOOR_2:' '{print $2}' | awk '{print $1}'"
                            tdkTestObj.addParameter("command", cmd)
                            expectedresult = "SUCCESS"

                            print("\nTEST STEP %d: Retrieve the value of WIFI_NOISE_FLOOR_2" % step)
                            print("EXPECTED RESULT %d: The value of WIFI_NOISE_FLOOR_2 should be retrieved" % step)
                            ...
                            if expectedresult in actualresult and details:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print("ACTUAL RESULT %d: Retrieved the value of WIFI_NOISE_FLOOR_2; Details : %s" % (step, details))
                                print("[TEST EXECUTION RESULT] : SUCCESS")
                            else:
                                tdkTestObj.setResultStatus("FAILURE")
                                print("ACTUAL RESULT %d: Failed to retrieve the value of WIFI_NOISE_FLOOR_2; Details : %s" % (step, details))
                                print("[TEST EXECUTION RESULT] : FAILURE")
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print("ACTUAL RESULT %d: WIFI_NOISE_FLOOR_2 marker is not found; Details : %s" %(step,details));
                            #Get the result of execution
                            print("[TEST EXECUTION RESULT] : FAILURE");
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
