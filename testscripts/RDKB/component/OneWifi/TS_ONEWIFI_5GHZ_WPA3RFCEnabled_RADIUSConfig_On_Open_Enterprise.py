...
<xml>
  ...
  <name>TS_ONEWIFI_5GHZ_WPA3RFCEnabled_RADIUSConfig_On_Open_Enterprise</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>When RFC WPA3-Personal-Transition is enabled check Radius configuration parameters on security mode transition(None-WPA3-Enterprise) for 5GHZ</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_268</test_case_id>
    <test_objective>When WPA3_Personal_Transition RFC Feature is enabled and When security mode transitions from None to WPA3-Enterprise (open-enterprise), check if the RADIUS configuration (Encryption method, Radius Server, Radius Port, Radius Secret) are updated. For 5GHZ</test_objective>
...
3. Get the security mode using Device.WiFi.AccessPoint.2.Security.ModeEnabled and store it.
4. Change security mode to None using Device.WiFi.AccessPoint.2.Security.ModeEnabled.
5. Check RADIUS config using Device.WiFi.AccessPoint.2.Security.RadiusServerIPAddr, Device.WiFi.AccessPoint.2.Security.RadiusServerPort and Device.WiFi.AccessPoint.2.Security.RadiusSecret. Check if the values are 0.0.0.0, 0 and empty respectively.
6. Change the security mode to WPA3-Enterprise, encryption method to AES, radius parameters: radius server, radius secret, radius port to valid values.
7. Check if security mode set is reflected in get and if the RADIUS configuration is set to the new values.
8. Revert to initial security mode.9. Unload the module</automation_approch>
    <expected_output>When security mode changes for open to enterprise(None to WPA3-Enterprise ) RADIUS config should reset.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WifiAgent</test_stub_interface>
    <test_script>TS_ONEWIFI_5GHZ_WPA3RFCEnabled_RADIUSConfig_On_Open_Enterprise</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_WPA3RFCEnabled_RADIUSConfig_On_Open_Enterprise')
sysobj.configureTestCase(ip,port,'TS_ONEWIFI_5GHZ_WPA3RFCEnabled_RADIUSConfig_On_Open_Enterprise')
        ...
        if expectedresult in actualresult:
            ...
            if pre_req_set == 1:
                ...
                if expectedresult in actualresult:
                    ...
                    #Check Security Mode and initial RADIUS Config
                    step = step + 1
                    ...
                    expected_values = ["None", "","0.0.0.0","0",""]
                    actual_values = []
                    actualresult_all = []
                    ...
                    print("TEST STEP %s: Check security mode and RADIUS configuration" % step)
                    print("EXPECTED RESULT %s: RADIUS configuration values should be reset when security mode is None" % step)

                    if "FAILURE" not in actualresult_all and actual_values == expected_values:
                        tdkTestObj.setResultStatus("SUCCESS")
                        print("ACTUAL RESULT %s: SecurityMode is '%s',Encryption Mode is ' %s', RadiusServerIp is ' %s', RadiusServerPort is ' %s', RadiusSecret is '%s'" % (step,actual_values[0],actual_values[1],actual_values[2],actual_values[3],actual_values[4]))
                        print("[TEST EXECUTION RESULT] : SUCCESS")

                        #Change Security Mode to WPA3-Enterprise, Encryption Mode to AES and radius config parameters to new values
                        step = step + 1
                        ...
                        sleep(2)
                        print(f"TEST STEP {step}: Change security mode to WPA3-Enterprise, Encryption mode to {ENCRYPTION_MODE}, RadiusServerIp to {RADIUS_IP} RadiusServerPort to {RADIUS_PORT} and RadiusSecret to {RADIUS_SECRET} ")
                        print("EXPECTED RESULT %s: Should set security mode to WPA3-Enterprise and Radius config parameters to new values" % step)

                        if expectedresult in actualresult:
                            tdkTestObj.setResultStatus("SUCCESS")
                            print("ACTUAL RESULT %s: Security mode set to WPA3-Enterprise, Radius config parameters are updated" % step)
                            print("[TEST EXECUTION RESULT] : SUCCESS")

                            #Check Security Mode and Radius Configuration
                            ...
                            for param in paramList_config:
                                ...
                                actualresult_all.append(actualresult)
                                actual_values.append(value)
                            print("TEST STEP %s: Check security mode and Radius configuration Values" % step)
                            print(f"EXPECTED RESULT {step}: Security mode should be WPA3-Enterprise, Encryption mode should be {ENCRYPTION_MODE}, RadiusServerIp to {RADIUS_IP} RadiusServerPort to {RADIUS_PORT} and RadiusSecret to {RADIUS_SECRET}")

                            if "FAILURE" not in actualresult_all and actual_values == expected_values:
                                tdkTestObj.setResultStatus("SUCCESS")
                                print("ACTUAL RESULT %s: Security mode is '%s', Encryption method is '%s', RadiusServerIp is ' %s', RadiusServerPort is ' %s', RadiusSecret is ' %s'" % (step,actual_values[0],actual_values[1],actual_values[2],actual_values[3],actual_values[4]))
                                print("[TEST EXECUTION RESULT] : SUCCESS")
                            else:
                                tdkTestObj.setResultStatus("FAILURE")
                                print("ACTUAL RESULT %s: Security mode is '%s', Encryption method is '%s', RadiusServerIp is ' %s', RadiusServerPort is ' %s', RadiusSecret is ' %s'" % (step,actual_values[0],actual_values[1],actual_values[2],actual_values[3],actual_values[4]))
                            ...
                            tdkTestObj.setResultStatus("FAILURE")
                            print("ACTUAL RESULT %s: Failed to change SecurityMode to WPA3-Enterprise and Radius configuration parameters to new values" % step)
                            print("[TEST EXECUTION RESULT] : FAILURE")
                    else:
                        tdkTestObj.setResultStatus("FAILURE")
                        print("ACTUAL RESULT %s: SecurityMode is '%s',Encryption Mode is ' %s', RadiusServerIp is ' %s', RadiusServerPort is ' %s', RadiusSecret is '%s'" % (step,actual_values[0],actual_values[1],actual_values[2],actual_values[3],actual_values[4]))
