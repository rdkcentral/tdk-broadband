        ...
        for i in range(1, 15):
            if markerfound == 1:
                ...
                if len(details) == 0 or "WIFI_RXCLIENTS_" not in details:
                    ...
                    marker_match = re.search(r"(WIFI_RXCLIENTS_[0-9]+):\s*([0-9]+)", details, re.IGNORECASE)
        ...
        if expectedresult in actualresult and markerfound == 1 and telemetryRXClientValue != "":
            ...
            for i in range(1, 15):
                if markerfound == 1:
                    ...
                    query = "cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_TXCLIENTS_\" | tail -1"
                    print("query:%s" % query)
                    ...
                    if len(details) == 0 or "WIFI_TXCLIENTS_" not in details:
                        ...
                        marker_match = re.search(r"(WIFI_TXCLIENTS_[0-9]+):\s*([0-9]+)", details, re.IGNORECASE)
