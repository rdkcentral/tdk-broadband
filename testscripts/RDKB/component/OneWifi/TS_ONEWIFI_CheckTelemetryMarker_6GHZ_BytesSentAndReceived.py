...
<xml>
  ...
  <name>TS_ONEWIFI_CheckTelemetryMarker_6GHZ_BytesSentAndReceived</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if the bytes sent and received via TR-181 and telemetry marker are greater than zero</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_302</test_case_id>
    <test_objective>This test case is to check if the bytes sent and received via TR-181 and telemetry marker are greater than zero</test_objective>
...
Device.WiFi.AccessPoint.17.AssociatedDevice.1.Stats.BytesSent
Device.WiFi.AccessPoint.17.AssociatedDevice.1.Stats.BytesReceived</input_parameters>
...
4.Check for telemetry marker WIFI_BYTESSENTCLIENTS_17 in wifihealth.txt and get the bytes sent using Device.WiFi.AccessPoint.17.AssociatedDevice.1.Stats.BytesSent  .
5.Check if the values received are greater than zero
6.Check for telemetry marker WIFI_BYTESRECEIVEDCLIENTS_17 in wifihealth.txt and get the bytes sent using Device.WiFi.AccessPoint.17.AssociatedDevice.1.Stats.BytesReceived.
...
9.Unload the Module
    ...
    <expected_output>Bytes sent and received via Tr-181 and telemetry markers must be greater than zero
</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiAgent</test_stub_interface>
    <test_script>TS_ONEWIFI_CheckTelemetryMarker_6GHZ_BytesSentAndReceived</test_script>
    <skipped></skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_CheckTelemetryMarker_6GHZ_BytesSentAndReceived')
sysObj.configureTestCase(ip,port,'TS_ONEWIFI_CheckTelemetryMarker_6GHZ_BytesSentAndReceived')
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in actualresult and tel_loginterval == "300":
                        ...
                        if expectedresult in actualresult and whl_found == 1:
                            ...
                            #Check for WIFI_BYTESSENTCLIENTS_17 Marker in wifihealth log file
                            step += 1
                            print(f"TEST STEP {step}: Check if WIFI_BYTESSENTCLIENTS_17 Marker is present in wifihealth log file")
                            print(f"EXPECTED RESULT {step}: WIFI_BYTESSENTCLIENTS_17 Marker should be present wifihealth log file")
                            markerfound = 0
                            for i in range(1, 6):
                                if markerfound == 1:
                                    break
                                else:
                                    print(f"Checking WIFI_BYTESSENTCLIENTS_17 in wifihealth log file iteration {i}")
                                    query = "cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_BYTESSENTCLIENTS_17:\""
                                    tdkTestObj = sysObj.createTestStep('ExecuteCmd')
                                    tdkTestObj.addParameter("command", query)
                                    tdkTestObj.executeTestCase(expectedresult)
                                    actualresult = tdkTestObj.getResult()
                                    details = tdkTestObj.getResultDetails()
                                    if (len(details) == 0) or details.endswith(":") or "WIFI_BYTESSENTCLIENTS_17" not in details:
                                        markerfound = 0
                                        sleep(60)
                                    else:
                                        tel_bytessent = details.split("WIFI_BYTESSENTCLIENTS_17:")[1].split(',')[0].replace("\\n", "").strip()
                                        markerfound = 1

                            if expectedresult in actualresult and markerfound == 1 and tel_bytessent != "":
                                tdkTestObj.setResultStatus("SUCCESS")
                                print(f"ACTUAL RESULT {step}: WIFI_BYTESSENTCLIENTS_17 Marker is {tel_bytessent}")
                                print("[TEST EXECUTION RESULT] : SUCCESS")

                                # Bytes Sent via TR-181
                                step += 1
                                tdkTestObj = obj.createTestStep('pam_GetParameterValues')
                                tdkTestObj.addParameter("ParamName", "Device.WiFi.AccessPoint.17.AssociatedDevice.1.Stats.BytesSent")
                                tdkTestObj.executeTestCase(expectedresult)
                                actualresult = tdkTestObj.getResult()
                                tr181_bytessent = tdkTestObj.getResultDetails()

                                print(f"TEST STEP {step}: Get the number of Bytes sent via TR-181 parameter")
                                print(f"EXPECTED RESULT {step}: Should get the number of Bytes sent via TR-181 parameter")

                                if expectedresult in actualresult:
                                    tdkTestObj.setResultStatus("SUCCESS")
                                    print(f"ACTUAL RESULT {step}: Number of Bytes sent via TR-181 parameter: {tr181_bytessent}")
                                    print("[TEST EXECUTION RESULT] : SUCCESS")

                                    # Verify Bytes sent > 0
                                    step += 1
                                    print(f"TEST STEP {step}: Check if Bytes sent via TR-181 parameter and telemetry marker are greater than zero")
                                    print(f"EXPECTED RESULT {step}: Should get the number of Bytes sent via TR-181 parameter and telemetry marker greater than zero")
                                    if int(tel_bytessent) > 0 and int(tr181_bytessent) > 0:
                                        tdkTestObj.setResultStatus("SUCCESS")
                                        print(f"ACTUAL RESULT {step}:  Bytes sent via TR-181: {tr181_bytessent}, Bytes sent via Telemetry marker: {tel_bytessent}")
                                        print("[TEST EXECUTION RESULT] : SUCCESS")

                                        # Check WIFI_BYTESRECEIVEDCLIENTS_17 Marker in wifihealth log
                                        step += 1
                                        print(f"TEST STEP {step}: Check if WIFI_BYTESRECEIVEDCLIENTS_17 Marker is present in wifihealth log")
                                        print(f"EXPECTED RESULT {step}: WIFI_BYTESRECEIVEDCLIENTS_17 Marker should be present in wifihealth log")
                                        markerfound = 0
                                        for i in range(1, 6):
                                            if markerfound == 1:
                                                break
                                            else:
                                                print(f"Checking WIFI_BYTESRECEIVEDCLIENTS_17 in wifihealth log file iteration {i}")
                                                query = "cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_BYTESRECEIVEDCLIENTS_17:\""
                                                tdkTestObj = sysObj.createTestStep('ExecuteCmd')
                                                tdkTestObj.addParameter("command", query)
                                                expectedresult = "SUCCESS"
                                                tdkTestObj.executeTestCase(expectedresult)
                                                actualresult = tdkTestObj.getResult()
                                                details = tdkTestObj.getResultDetails()
                                                if (len(details) == 0) or details.endswith(":") or "WIFI_BYTESRECEIVEDCLIENTS_17" not in details:
                                                    markerfound = 0
                                                    sleep(60)
                                                else:
                                                    tel_bytesreceived = details.split("WIFI_BYTESRECEIVEDCLIENTS_17:")[1].split(',')[0].replace("\\n", "").strip()
                                                    markerfound = 1
                                        if expectedresult in actualresult and markerfound == 1 and tel_bytesreceived != "":
                                            tdkTestObj.setResultStatus("SUCCESS")
                                            print(f"ACTUAL RESULT {step}: WIFI_BYTESRECEIVEDCLIENTS_17 Marker is {tel_bytesreceived}")
                                            print("[TEST EXECUTION RESULT] : SUCCESS")
                                            # Bytes Received via TR-181
                                            step += 1
                                            tdkTestObj = obj.createTestStep('pam_GetParameterValues')
                                            tdkTestObj.addParameter("ParamName", "Device.WiFi.AccessPoint.17.AssociatedDevice.1.Stats.BytesReceived")
                                            tdkTestObj.executeTestCase(expectedresult)
                                            actualresult = tdkTestObj.getResult()
                                            tr181_bytesreceived = tdkTestObj.getResultDetails()

                                            print(f"TEST STEP {step}: Get the number of Bytes received via TR-181 parameter")
                                            print(f"EXPECTED RESULT {step}: Should get the number of Bytes received via TR-181 parameter")

                                            if expectedresult in actualresult:
                                                tdkTestObj.setResultStatus("SUCCESS")
                                                print(f"ACTUAL RESULT {step}: Number of Bytes received via TR-181 parameter: {tr181_bytesreceived}")
                                                print("[TEST EXECUTION RESULT] : SUCCESS")

                                                # Verify Bytes received > 0
                                                step += 1
                                                print(f"TEST STEP {step}: Check if Bytes received via TR-181 parameter and telemetry marker are greater than zero")
                                                print(f"EXPECTED RESULT {step}: Bytes received via TR-181 parameter and telemetry marker should be greater than zero")
                                                if int(tel_bytesreceived) > 0 and int(tr181_bytesreceived) > 0:
                                                    tdkTestObj.setResultStatus("SUCCESS")
                                                    print(f"ACTUAL RESULT {step}: Bytes received via TR-181: {tr181_bytesreceived},Bytes received via Telemetry marker: {tel_bytesreceived}")
                                                    print("[TEST EXECUTION RESULT] : SUCCESS")
                                                else:
                                                    tdkTestObj.setResultStatus("FAILURE")
                                                    print(f"ACTUAL RESULT {step}: Bytes received via TR-181: {tr181_bytesreceived}, Bytes received via Telemetry marker: {tel_bytesreceived}")
                                                ...
                                                tdkTestObj.setResultStatus("FAILURE")
                                                print(f"ACTUAL RESULT {step}: Bytes received via TR-181 parameter: {tr181_bytesreceived}")
                                                print("[TEST EXECUTION RESULT] : FAILURE")
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE")
                                            print(f"ACTUAL RESULT {step}: WIFI_BYTESRECEIVEDCLIENTS_17 Marker not present")
                                            print("[TEST EXECUTION RESULT] : FAILURE")
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE")
                                        print(f"ACTUAL RESULT {step}: Bytes sent via TR-181: {tr181_bytessent}, Bytes sent via Telemetry marker: {tel_bytessent}")
                                ...
                                tdkTestObj.setResultStatus("FAILURE")
                                print(f"ACTUAL RESULT {step}: WIFI_BYTESSENTCLIENTS_17 Marker not found in wifihealth log file")
                                print("[TEST EXECUTION RESULT] : FAILURE")
                        else:
                            tdkTestObj.setResultStatus("FAILURE")
