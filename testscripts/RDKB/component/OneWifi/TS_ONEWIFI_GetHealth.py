...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>6</version>
  <name>TS_ONEWIFI_GetHealth</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>TC_ONEWIFI_9</test_case_id>
...
2.Python Script will be generated/overrided automatically by Test Manager with provided arguments in configure page (TS_ONEWIFI_GetHealth.py)
3.Execute the generated Script(TS_ONEWIFI_GetHealth.py) using execution page of  Test Manager GUI
...
7.wifiagentstub will validate the available result (from agent console log and Pointer to instance as updated) with expected result (eg:"Health of ONEWIFI " along with info) and  the same is updated to agent console log.
...
Health of ONEWIFI log from DUT should be available in Agent Console Log
CheckPoint 2:
...
Test Manager GUI will publish the result as SUCCESS in Execution page</except_output>
    ...
    <test_script>TS_ONEWIFI_GetHealth</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ONEWIFI_GetHealth');
