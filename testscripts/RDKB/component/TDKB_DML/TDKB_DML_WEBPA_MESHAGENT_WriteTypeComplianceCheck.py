...
ip = <ipaddress>
port = <port>
obj1.configureTestCase(ip,port,'TDKB_DML_WEBPA_MESHAGENT_WriteTypeComplianceCheck')
obj2.configureTestCase(ip,port,'TDKB_DML_WEBPA_MESHAGENT_WriteTypeComplianceCheck')
rbusobj.configureTestCase(ip,port,'TDKB_DML_WEBPA_MESHAGENT_WriteTypeComplianceCheck')
...
if "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper() and "SUCCESS" in loadmodulestatus3.upper():
    ...
    setup_type = "WEBPA"
    factoryReset = "false"
    #Invoke the utility function to check if write operations are write-ype compliant
    failedParams, moduleStatus = tdkbDmlUtility.getAllParams_module(tdkbDmlModuleList.MESHAGENT, setup_type, factoryReset, [obj1, obj2], rbusobj, "writeTypeCompliance")
