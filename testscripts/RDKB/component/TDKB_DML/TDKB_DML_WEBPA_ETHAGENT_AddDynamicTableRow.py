...
ip = <ipaddress>
port = <port>
obj1.configureTestCase(ip,port,'TDKB_DML_WEBPA_ETHAGENT_AdddDynamicTableRow')
obj2.configureTestCase(ip,port,'TDKB_DML_WEBPA_ETHAGENT_AddDynamicTableRow')
rbusobj.configureTestCase(ip,port,'TDKB_DML_WEBPA_ETHAGENT_AddDynamicTableRow')
...
if "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper() and "SUCCESS" in loadmodulestatus3.upper():
    ...
    setup_type = "WEBPA"
    factoryReset = "false"
    #Invoke the utility function to check if adding rows to dynamic table returns failure
    failedtables, moduleStatus = tdkbDmlUtility.getAllParams_module(tdkbDmlModuleList.ETHAGENT, setup_type, factoryReset, [obj1, obj2], rbusobj, "AddDynamicTableRow")
