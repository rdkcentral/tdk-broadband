...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult and Initial_Mode:
        ...
        if "bridge mode" not in Mode:
            ...
            print(f"\nThe device is already in bridge mode, no set operation required")
        ...
        if exitFlag != 1:
            ...
            if expectedresult in actualresult:
                #Set the result status of execution
                tdkTestObj.setResultStatus("SUCCESS")
                print(f"ACTUAL RESULT {step}: Got the number of hosts as {NoOfHosts}")
                #Get the result of execution
                print("[TEST EXECUTION RESULT] : SUCCESS")
                ...
                if NoOfHosts.isdigit() and int(NoOfHosts) == 0:
                    ...
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("FAILURE")
                    print(f"ACTUAL RESULT{step}: Failed to get number of Hosts as zero and Number of Hosts is {NoOfHosts}")
                    #Get the result of execution
                    print("[TEST EXECUTION RESULT] : FAILURE")
            else:
                #Set the result status of execution
                tdkTestObj.setResultStatus("FAILURE")
                print(f"ACTUAL RESULT {step}: No of Hosts is {NoOfHosts}")
                #Get the result of execution
                print("[TEST EXECUTION RESULT] : FAILURE")
        if revertFlag == 1:
            step += 1
            ...
            print(f"\nRevert of Lan Mode is  not required\n")
    else:
        #Set the result status of execution
        tdkTestObj.setResultStatus("FAILURE")
