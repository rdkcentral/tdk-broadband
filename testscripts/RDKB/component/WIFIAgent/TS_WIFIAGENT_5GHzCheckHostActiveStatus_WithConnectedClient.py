...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WIFIAGENT_5GHzCheckHostActiveStatus_WithConnectedClient</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Check if the Host Active status Device.Hosts.Host.{i}.Active is "true" for 5G connected client.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_229</test_case_id>
    <test_objective>Check if the Host Active status Device.Hosts.Host.{i}.Active is "true" for 5G connected client.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband, RPI</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script
3. Connect a 5G wifi client</pre_requisite>
...
4. Find the Host Table entry for wifi client by checking if Device.Hosts.Host.{i}.Layer1Interface is equal to Device.WiFi.SSID.2.
5. For the Host Entry of wifi client, query Device.Hosts.Host.{i}.Active and check if it is Active. Else, return failure.
6. If none of the host entry's layer 1 interface gives "Device.WiFi.SSID.2", return failure.
</automation_approch>
    <expected_output>The Host Active status Device.Hosts.Host.{i}.Active should be "true" for 5G connected client.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_5GHzCheckHostActiveStatus_WithConnectedClient</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_5GHzCheckHostActiveStatus_WithConnectedClient');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult and hostEntries.isdigit():
        ...
        #Iterate through the host table and find the index for which Layer1Interface is Device.WiFi.SSID.2
        clientDetected = 0;
        for index in range(1, hostEntries + 1):
            ...
            print("\nTEST STEP %d : Get the value of %s and check if it is Device.WiFi.SSID.2" %(step, paramName));
            ...
            if expectedresult in actualresult and layer1Interface != "":
                ...
                if layer1Interface == "Device.WiFi.SSID.2":
                    clientDetected = 1;
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("SUCCESS");
        ...
        if clientDetected != 1:
            #Set the result status of execution
            tdkTestObj.setResultStatus("FAILURE");
            print("\nNone of the Host Table entries show Device.WiFi.SSID.2 as Layer 1 Interface");
    else:
        #Set the result status of execution
        tdkTestObj.setResultStatus("FAILURE");
        print("ACTUAL RESULT %d : Device.Hosts.HostNumberOfEntries : %d" %(step, hostEntries));
