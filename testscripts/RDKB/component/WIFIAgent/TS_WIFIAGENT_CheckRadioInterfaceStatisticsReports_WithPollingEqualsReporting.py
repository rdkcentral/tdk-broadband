...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIAGENT_CheckRadioInterfaceStatisticsReports_WithPollingEqualsReporting</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Radio Interface Statistics Harvester reports are properly generated when the reporting and polling intervals are configured.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_232</test_case_id>
    <test_objective>To check if the Radio Interface Statistics Harvester reports are properly generated when the reporting and polling intervals are configured.</test_objective>
...
paramName: Device.X_RDKCENTRAL-COM_Report.RadioInterfaceStatistics.ReportingPeriod
paramValue: 30/60
paramType: unsigned int
...
4. Get the initial Harvester configuration parameters for Radio Interface Statistics Report and save them: Device.X_RDKCENTRAL-COM_Report.RadioInterfaceStatistics.PollingPeriod, Device.X_RDKCENTRAL-COM_Report.RadioInterfaceStatistics.ReportingPeriod, Device.X_RDKCENTRAL-COM_Report.RadioInterfaceStatistics.Enabled
5. To check the initial number of reports sent, check the number of log lines "event:raw.kestrel.reports.RadioInterfacesStatistics" in PARODUSlog.txt.0
6. Before setting new intervals, disable the report generation by setting Device.X_RDKCENTRAL-COM_Report.RadioInterfaceStatistics.Enabled to false and validate with get.
7. Set Device.X_RDKCENTRAL-COM_Report.RadioInterfaceStatistics.PollingPeriod and Device.X_RDKCENTRAL-COM_Report.RadioInterfaceStatistics.ReportingPeriod to equal values (either both as 30 or 60 - chosen dynamically which is different from the initial value). Validate the SET with GET.
8. Enable Device.X_RDKCENTRAL-COM_Report.RadioInterfaceStatistics.Enabled by setting it to true. Validate SET with GET.
9. Now sleep for twice the polling (or reporting) interval set.
10. Get the final number of reports generated by checking the number of log lines "event:raw.kestrel.reports.RadioInterfacesStatistics" in PARODUSlog.txt.0.
11. Compute the difference between the final and initial number of log lines and the difference should be &gt;= 1.
12. Before reverting the intervals to initial values, disable the report generation by setting Device.X_RDKCENTRAL-COM_Report.RadioInterfaceStatistics.Enabled to false and validate with GET.
...
15. Unload the modules.
    ...
    <expected_output>Radio Interfaces Statistics Harvester reports should be properly generated when the reporting and polling intervals are configured.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckRadioInterfaceStatisticsReports_WithPollingEqualsReporting</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckRadioInterfaceStatisticsReports_WithPollingEqualsReporting');
sysObj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckRadioInterfaceStatisticsReports_WithPollingEqualsReporting');
