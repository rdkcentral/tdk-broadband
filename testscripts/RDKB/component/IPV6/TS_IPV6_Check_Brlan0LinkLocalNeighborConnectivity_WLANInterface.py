...
tr181obj.configureTestCase(ip,port,'TS_IPV6_Check_Brlan0LinkLocalNeighborConnectivity_WLANInterface')
sysobj.configureTestCase(ip,port,'TS_IPV6_Check_Brlan0LinkLocalNeighborConnectivity_WLANInterface')
    ...
    if interface != "":
        ...
        if flag:
            ...
            if index is not None:
                ...
                if flag:
                    step += 1
                    print(f"Check whether {interface} can ping the link-local IPv6 address of the active WLAN client")
                    ping_flag = checkInternetConnectivity(sysobj, link_local_ipv6, PING_COUNT, step, interface)
                    if ping_flag:
                        print(f"The {interface} interface can ping the link-local IPv6 address of the active WLAN client successfully")
                    else:
                        print(f"The {interface} interface cannot ping the link-local IPv6 address of the active WLAN client")
                else:
                    print("Failed to get the link-local IPv6 address of the active WLAN client")
            else:
                print("Failed to get the index of the active WLAN client")
        else:
            print(f"The {interface} interface does not have inet6 address with link local scope")
    else:
        print(f"Failed to get the LAN interface name from variables file.")
